...
- Branch: `feature/epra-prototype-rank-alignment`
- Parent protocol: `protocol/fixed-eval-panel` at `fdbfffd`
...
- Fixed-panel A2 reference: Small-LI `0.45507`, Large-LI `0.37143`
...
One fixed variant advances only if its completed 500-epoch selected Test F1 is
at least `0.46007` on Small-LI and `0.37643` on Large-LI. Raw-best is diagnostic
only. The winning fixed strength, not a dataset-wise mixture, is used for all
later datasets.
