...
A2 is the initial dynamic-random reference. Previous post-hoc corrections failed mainly because
...
- Dynamic-random code version and one hyperparameter set for both datasets.
- Small-LI seed 42 and Large-LI seed 44.
- 500 epochs, no early stopping.
- Dynamic random train/validation/test sampling, with `shuffle=True`.
- Primary endpoint: test F1 at the best validation-F1 epoch.
- Required Val-selected improvement over initial A2: at least `+0.005` on both datasets.
- A2 references: Small-LI `0.46247`, Large-LI `0.30108`.
- Raw-best is compared only with A2 Raw-best (`0.50667`, `0.44720`).
- Deltas below `0.005` are marked as possible sampling fluctuation.
...
Use `run/dynamic_random_pair_queue.py` with:

```bash
 DYNAMIC_SPEC=run/dynamic_random_pair_spec.json \
PAIR_GPU_ALLOWLIST=3,4 \
PAIR_OUT_BASE=/e/yky/FraudGT_pair_results \
PAIR_RUNTIME_BASE=/e/yky/FraudGT_pair_runtime \
python run/dynamic_random_pair_queue.py
```

Audit with `run/dynamic_random_result_audit.py`; the only reference is
