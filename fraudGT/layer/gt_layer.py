...
class GTLayer(nn.Module):
    ...
    def __init__(self, dim_in, dim_h, dim_out, metadata, local_gnn_type, global_model_type, index, num_heads=1,
        ...
        self.directional_meanmaxwinnerprojflowgate_writeback = (
            global_model_type == 'SparseNodeTransformer' and
            cfg.gt.edge_writeback == 'dir_meanmaxwinnerprojflowgate'
        )
            ...
            if self.edge_writeback_enabled:
                ...
                writeback_context_dim = dim_out * (
                    4 if (
                        ...
                        self.directional_meanmaxwinnerprojflowgate_writeback or
                ...
                if self.directional_meanmaxwinnerprojflowgate_writeback:
                    self.writeback_winner_proj_add = nn.Parameter(
                        torch.full((2,), math.log(0.15 / 0.85))
                    )
                    self.writeback_flow_gate_scale = nn.Parameter(
                        torch.full((1,), 2.0)
                    )
                    self.writeback_flow_gate_bias = nn.Parameter(
                        torch.full((1,), -2.0)
                    )
    ...
    def _apply_edge_writeback(self, out, edge_state, src_nodes, dst_nodes,
        ...
        if (
            ...
            if self.directional_meansoftmax_writeback:
                ...
                if self.directional_meanmax_writeback:
                ...
                elif self.directional_meanmaxwinnerprojflowgate_writeback:
                    anomaly_scores = weighted_edge_state.norm(dim=-1)
                    incoming_winner = self._group_top1_select(
                        weighted_edge_state, dst_nodes, anomaly_scores, num_nodes
                    )
                    ...
                    incoming_proj = F.relu(incoming_proj_coeff) * incoming_spike
                    outgoing_proj = F.relu(outgoing_proj_coeff) * outgoing_spike
                    incoming_proj_norm = incoming_proj.norm(dim=-1, keepdim=True)
                    outgoing_proj_norm = outgoing_proj.norm(dim=-1, keepdim=True)
                    flow_cos = F.cosine_similarity(
                        incoming_proj, outgoing_proj, dim=-1, eps=1e-6
                    ).unsqueeze(-1)
                    flow_strength = torch.log1p(
                        torch.sqrt(incoming_proj_norm * outgoing_proj_norm)
                    )
                    flow_gate = torch.sigmoid(
                        self.writeback_flow_gate_scale.view(1, 1) *
                        flow_cos * flow_strength +
                        self.writeback_flow_gate_bias.view(1, 1)
                    )
                    edge_context = torch.cat(
                        (
                            incoming,
                            outgoing,
                            incoming_max +
                            winner_proj_add[0] * (1.0 + flow_gate) *
                            incoming_proj,
                            outgoing_max +
                            winner_proj_add[1] * (1.0 + flow_gate) *
                            outgoing_proj,
                        ),
                        dim=-1
                    )
