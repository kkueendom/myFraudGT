...
class GTLayer(nn.Module):
    ...
    def __init__(self, dim_in, dim_h, dim_out, metadata, local_gnn_type, global_model_type, index, num_heads=1,
        ...
        self.directional_meanmaxtopkprojpluswinnershape_writeback = (
            global_model_type == 'SparseNodeTransformer' and
            cfg.gt.edge_writeback == 'dir_meanmaxtopkprojpluswinnershape'
        )
            ...
            if self.edge_writeback_enabled:
                ...
                writeback_context_dim = dim_out * (
                    4 if (
                        ...
                        self.directional_meanmaxtopkprojpluswinnershape_writeback or
                ...
                if self.directional_meanmaxtopkprojpluswinnershape_writeback:
                    self.writeback_focus_proj_add = nn.Parameter(
                        torch.full((2,), math.log(0.15 / 0.85))
                    )
                    self.writeback_winner_resid_add = nn.Parameter(
                    ...
                    self.writeback_shape_resid_add = nn.Parameter(
                        torch.full((2,), math.log(0.05 / 0.95))
                    )
                    self.writeback_shape_tau = nn.Parameter(
                        torch.full((2,), 1.0)
                    )
    ...
    def _apply_edge_writeback(self, out, edge_state, src_nodes, dst_nodes,
        ...
        if (
            ...
            if self.directional_meansoftmax_writeback:
                ...
                if self.directional_meanmax_writeback:
                ...
                elif self.directional_meanmaxtopkprojpluswinnershape_writeback:
                    anomaly_scores = weighted_edge_state.norm(dim=-1)
                    incoming_focus = self._group_topk_mean(
                        weighted_edge_state, dst_nodes, anomaly_scores, num_nodes
                    )
                    ...
                    focus_proj_add = torch.sigmoid(self.writeback_focus_proj_add)
                    winner_resid_add = torch.sigmoid(self.writeback_winner_resid_add)
                    shape_resid_add = torch.sigmoid(self.writeback_shape_resid_add)
                    shape_tau = F.softplus(self.writeback_shape_tau) + 1e-6
                    incoming_spike = incoming_max - incoming
                    outgoing_spike = outgoing_max - outgoing
                    incoming_focus_residual = incoming_focus - incoming
                    outgoing_focus_residual = outgoing_focus - outgoing
                    ...
                    incoming_shape_delta = (
                        incoming_winner_proj - incoming_focus_proj
                    )
                    outgoing_shape_delta = (
                        outgoing_winner_proj - outgoing_focus_proj
                    )
                    incoming_shape_gate = F.relu(
                        F.cosine_similarity(
                            incoming_winner_residual,
                            incoming_focus_residual,
                            dim=-1,
                            eps=1e-6,
                        )
                    ).unsqueeze(-1)
                    outgoing_shape_gate = F.relu(
                        F.cosine_similarity(
                            outgoing_winner_residual,
                            outgoing_focus_residual,
                            dim=-1,
                            eps=1e-6,
                        )
                    ).unsqueeze(-1)
                    incoming_shape_clip = (
                        shape_tau[0] *
                        torch.tanh(incoming_shape_delta / shape_tau[0])
                    )
                    outgoing_shape_clip = (
                        shape_tau[1] *
                        torch.tanh(outgoing_shape_delta / shape_tau[1])
                    )
                    edge_context = torch.cat(
                        (
                            incoming,
                            ...
                            winner_resid_add[0] * incoming_winner_proj +
                            shape_resid_add[0] *
                            incoming_shape_gate * incoming_shape_clip,
                            outgoing_max +
                            focus_proj_add[1] * outgoing_focus_proj +
                            winner_resid_add[1] * outgoing_winner_proj +
                            shape_resid_add[1] *
                            outgoing_shape_gate * outgoing_shape_clip,
                        ),
                        dim=-1
                    )
