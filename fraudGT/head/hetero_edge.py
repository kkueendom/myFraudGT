...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqpairseqbridgebankmotifpair',
        }
        ...
        self.use_sequence_bridge_motif_pair = (
            self.edge_decoding == 'pair_chain_contextseqpairseqbridgebankmotifpair'
        )
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_sequence_context_residual:
                ...
                if self.use_sequence_bridge_bank:
                    ...
                    if self.use_sequence_bridge_motif_pair:
                        self.bridge_path_pair_proj = MLP(dim_in * 3 + 1, dim_in,
                                                         num_layers=self.head_layers,
                                                         bias=True)
                        self.bridge_path_bank_proj = MLP(dim_in * 3, dim_in,
                                                         num_layers=self.head_layers,
                                                         bias=True)
                        self.bridge_path_bank_gate = nn.Linear(dim_in * 3, dim_in)
                        self.bridge_path_bank_update = MLP(dim_in * 3, dim_in,
                                                           num_layers=self.head_layers,
                                                           bias=True)
                        self.bridge_path_bank_alpha = nn.Parameter(
    ...
    def _recent_overlap_path_repr(self, bank_a, bank_b, seq_a, seq_b, time_a, time_b):
        valid_a = bank_a >= 0
        valid_b = bank_b >= 0
        eq = (
            bank_a.unsqueeze(2) == bank_b.unsqueeze(1)
        ) & valid_a.unsqueeze(2) & valid_b.unsqueeze(1)
        time_align = torch.exp(
            -(
                time_a.unsqueeze(2) - time_b.unsqueeze(1)
            ).abs() / self.sequence_time_scale.abs().clamp(min=1.0)
        ) * eq.float()
        token_input = torch.cat(
            (
                seq_a.unsqueeze(2).expand(-1, -1, seq_b.size(1), -1),
                seq_b.unsqueeze(1).expand(-1, seq_a.size(1), -1, -1),
                (
                    seq_a.unsqueeze(2) * seq_b.unsqueeze(1)
                ),
                time_align.unsqueeze(-1),
            ),
            dim=-1,
        )
        flat_token = self.bridge_path_pair_proj(
            token_input.reshape(-1, token_input.size(-1))
        ).reshape(token_input.size(0), token_input.size(1), token_input.size(2), -1)
        weighted_token = flat_token * time_align.unsqueeze(-1)
        match_mass = time_align.sum(dim=(1, 2), keepdim=True)
        mean_token = weighted_token.sum(dim=(1, 2)) / match_mass.clamp(min=1e-6)
        max_token = flat_token.masked_fill(~eq.unsqueeze(-1), -1e9).amax(dim=(1, 2))
        has_match = eq.any(dim=(1, 2))
        max_token = torch.where(
            has_match.unsqueeze(-1),
            max_token,
            torch.zeros_like(max_token),
        )
        return self.bridge_path_bank_proj(torch.cat(
            (
                mean_token,
                max_token,
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                if self.use_sequence_bridge_bank:
                    ...
                    if self.use_sequence_bridge_motif_pair:
                        ...
                        path_gate = torch.sigmoid(self.bridge_path_bank_gate(path_input))
                        pair_sequence_repr = pair_sequence_repr + (
                            torch.sigmoid(self.bridge_path_bank_alpha) *
                            path_gate *
                            self.bridge_path_bank_update(path_input)
                        )
