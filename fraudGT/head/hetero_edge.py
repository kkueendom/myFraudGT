...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqbridgebankresid',
        }
        ...
        self.use_sequence_bridge_bank_residual = (
            self.edge_decoding == 'pair_chain_contextseqbridgebankresid'
        )
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_sequence_context_residual:
                ...
                if self.use_sequence_bridge_bank_residual:
                    ...
                    self.bridge_bank_proj = MLP(dim_in * 3, dim_in,
                                                num_layers=self.head_layers,
                                                bias=True)
                    self.bridge_bank_fuse = MLP(dim_in * 3, dim_in,
                                                num_layers=self.head_layers,
                                                bias=True)
                    self.bridge_bank_head = MLP(dim_in, dim_out,
                                                num_layers=self.head_layers,
                                                bias=True)
                    self.bridge_bank_residual_alpha = nn.Parameter(
                        torch.full((1,), math.log(0.10 / 0.90))
                    )
        else:
            self.layer_post_mp = MLP(dim_in * 3, dim_out,
    ...
    def _pair_chain_head(self, batch):
        ...
        if task[0] == task[2]:
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                if self.use_sequence_bridge_bank_residual:
                    ...
                    pair_bridge_repr = self.bridge_bank_fuse(torch.cat(
                        (
                            bridge_context,
                            pair_sequence_repr,
        ...
        if self.use_sequence_context_residual:
            ...
            if self.use_sequence_bridge_bank_residual:
                ...
                bridge_logits = self.bridge_bank_head(pair_bridge_repr[pair_inv][mask])
                pred = pred + (
                    torch.sigmoid(self.bridge_bank_residual_alpha) * bridge_logits
                )
