...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqpairseqbridgebankwindowseqselectxalignoverlapgate',
        }
        ...
        self.use_sequence_cross_align_overlap_gate = self.use_sequence_cross_align
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_sequence_context_residual:
                ...
                if self.use_sequence_cross_align:
                    ...
                    if self.use_sequence_cross_align_overlap_gate:
                        self.sequence_cross_align_pair_gate = MLP(
                            dim_in + 2, 1,
                            num_layers=self.head_layers,
                            bias=True,
                        )
                        self.sequence_cross_align_pair_alpha = nn.Parameter(
                            torch.full((1,), math.log(0.25 / 0.75))
                        )
    ...
    def _cross_align_sequence_banks(self, source_bank, target_bank, pair_repr, pair_gate=None):
    ...
    def _recent_overlap_stats(self, bank_a, bank_b):
        ...
        match_any = eq.any(dim=2).float() * valid_a.float()
        overlap_count = match_any.sum(dim=1, keepdim=True)
        valid_count = valid_a.float().sum(dim=1, keepdim=True).clamp(min=1.0)
        slot_index = torch.arange(
            bank_a.size(1), device=bank_a.device, dtype=torch.float32
        )
        slot_weight = (1.0 / (1.0 + slot_index)).unsqueeze(0)
        weighted_overlap = (
            match_any * slot_weight
        ).sum(dim=1, keepdim=True) / slot_weight.sum().clamp(min=1e-6)
        overlap_ratio = overlap_count / valid_count
        return overlap_ratio, weighted_overlap
        ...
        if task[0] == task[2]:
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                if self.use_sequence_cross_align_overlap_gate:
                    ...
                    overlap_ratio, weighted_overlap = self._recent_overlap_stats(
                        outgoing_partner_bank[pair_src],
                        incoming_partner_bank[pair_dst],
                    )
                    pair_cross_align_gate = torch.sigmoid(
                        self.sequence_cross_align_pair_gate(
                            torch.cat((pair_repr, overlap_ratio, weighted_overlap), dim=-1)
                        )
                    )
                    pair_cross_align_alpha = torch.sigmoid(self.sequence_cross_align_pair_alpha)
                    pair_cross_align_gate = (
                        1.0 - pair_cross_align_alpha +
                        pair_cross_align_alpha * pair_cross_align_gate
                    ).unsqueeze(-1)
                if self.use_sequence_bridge_bank_window:
                    base_time_scale = self.sequence_time_scale.abs().clamp(min=1.0)
                    fast_time_scale = base_time_scale * self.fast_time_scale_log.exp().clamp(min=0.1, max=10.0)
