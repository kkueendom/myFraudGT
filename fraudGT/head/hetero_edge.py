...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqpairseqbridgebankdiff',
        }
        ...
        self.use_sequence_bridge_diff = (
            self.edge_decoding == 'pair_chain_contextseqpairseqbridgebankdiff'
        )
            ...
            if self.use_sequence_context_residual:
                ...
                if self.use_sequence_bridge_bank:
                    self.bridge_partner_proj = MLP(dim_in * 2 + 2, dim_in,
                                                   num_layers=self.head_layers,
                                                   bias=True)
                    bridge_bank_in_dim = dim_in * 4 if self.use_sequence_bridge_diff else dim_in * 3
                    self.bridge_bank_proj = MLP(bridge_bank_in_dim, dim_in,
                                                num_layers=self.head_layers,
                                                bias=True)
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                if self.use_sequence_bridge_bank:
                    ...
                    bridge_context = self.bridge_bank_proj(torch.cat(
                        ...
                        ) if self.use_sequence_bridge_diff else (
                            forward_bridge,
                            cycle_bridge,
                            forward_bridge * cycle_bridge,
                        ),
