...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqpairseqbridgebankattn',
        }
        ...
        self.use_sequence_bridge_attn = (
            self.edge_decoding == 'pair_chain_contextseqpairseqbridgebankattn'
        )
            ...
            if self.use_sequence_context_residual:
                ...
                if self.use_sequence_bridge_bank:
                    self.bridge_partner_proj = MLP(dim_in * 2 + 2, dim_in,
                                                   num_layers=self.head_layers,
                                                   bias=True)
                    if self.use_sequence_bridge_attn:
                        self.bridge_token_proj = MLP(dim_in, dim_in,
                                                     num_layers=self.head_layers,
                                                     bias=True)
                        self.bridge_attn_q = nn.Linear(dim_in, dim_in)
                        self.bridge_attn_k = nn.Linear(dim_in, dim_in)
                        self.bridge_attn_v = nn.Linear(dim_in, dim_in)
                        self.bridge_bank_proj = MLP(dim_in * 2, dim_in,
                                                    num_layers=self.head_layers,
                                                    bias=True)
                    else:
                    ...
                    if self.use_sequence_bridge_attn:
                        bridge_tokens = torch.stack(
                            (
                                ...
                                torch.abs(forward_bridge - cycle_bridge),
                            ),
                            dim=1,
                        )
                        flat_bridge_tokens = bridge_tokens.reshape(
                            -1, bridge_tokens.size(-1)
                        )
                        flat_bridge_tokens = self.bridge_token_proj(flat_bridge_tokens)
                        bridge_tokens = flat_bridge_tokens.view(
                            pair_sequence_repr.size(0), 4, -1
                        )
                        bridge_query = self.bridge_attn_q(pair_sequence_repr).unsqueeze(1)
                        bridge_keys = self.bridge_attn_k(bridge_tokens)
                        bridge_values = self.bridge_attn_v(bridge_tokens)
                        bridge_attn = torch.softmax(
                            (bridge_query * bridge_keys).sum(dim=-1) /
                            math.sqrt(bridge_keys.size(-1)),
                            dim=1,
                        ).unsqueeze(-1)
                        bridge_focus = (bridge_attn * bridge_values).sum(dim=1)
                        bridge_mean = bridge_tokens.mean(dim=1)
                        bridge_context = self.bridge_bank_proj(torch.cat(
                            (bridge_focus, bridge_mean),
                            dim=-1,
                        ))
                    else:
                        bridge_context = self.bridge_bank_proj(torch.cat(
