...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_correlation_sequence = self.edge_decoding == 'pair_chain_contextseqpaircorr'
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_pair_correlation_sequence:
                self.pair_history_encoder = nn.GRU(
                    input_size=dim_in,
                    hidden_size=dim_in,
                    batch_first=True,
                )
                self.pair_correlation_proj = MLP(dim_in * 3, dim_in,
                                                 num_layers=self.head_layers,
                                                 bias=True)
                self.pair_correlation_gate = nn.Linear(dim_in * 3, dim_in)
        else:
            self.layer_post_mp = MLP(dim_in * 3, dim_out,
                                     num_layers=self.head_layers,
                                     bias=True)
    ...
    def _build_pair_history_bank(self, edge_repr, pair_inv, edge_timestamps,
                                 num_pairs, latest_pair_timestamps):
        seq_bank = edge_repr.new_zeros((num_pairs, self.sequence_len, edge_repr.size(-1)))
    ...
    def _pair_chain_head(self, batch):
        ...
        if task[0] == task[2]:
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                pair_history_state = None
                if self.use_pair_correlation_sequence:
                    pair_history_bank = self._build_pair_history_bank(
                        edge_repr, pair_inv, edge_timestamps, num_pairs, pair_timestamps
                    )
                    pair_history_state = self.pair_history_encoder(pair_history_bank)[1].squeeze(0)
                ...
                if pair_history_state is not None:
                    pair_correlation_input = torch.cat(
                        (
                            outgoing_state,
                            incoming_state,
                            pair_history_state,
                    ...
                    pair_correlation_gate = torch.sigmoid(
                        self.pair_correlation_gate(pair_correlation_input)
                    )
                    sequence_interaction = (
                        pair_correlation_gate *
                        self.pair_correlation_proj(pair_correlation_input) +
                        (1.0 - pair_correlation_gate) * sequence_interaction
                    )
