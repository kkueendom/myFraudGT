...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqslotresid',
        }
        ...
        self.use_sequence_slot_residual = (
            self.edge_decoding == 'pair_chain_contextseqslotresid'
        )
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_sequence_context_residual:
                ...
                if self.use_sequence_slot_residual:
                    self.slot_sequence_encoder = nn.GRU(
                        input_size=dim_in,
                        hidden_size=dim_in,
                        batch_first=True,
                    )
                    self.slot_sequence_proj = MLP(dim_in * 3, dim_in,
                                                  num_layers=self.head_layers,
                                                  bias=True)
                    self.slot_sequence_head = MLP(dim_in, dim_out,
                                                  num_layers=self.head_layers,
                                                  bias=True)
                    self.slot_sequence_residual_alpha = nn.Parameter(
                        torch.full((1,), math.log(0.05 / 0.95))
                    )
                    self.slot_sequence_token = nn.Linear(dim_in * 3, dim_in)
        else:
            self.layer_post_mp = MLP(dim_in * 3, dim_out,
                                     num_layers=self.head_layers,
                                     bias=True)
    ...
    def _pair_chain_head(self, batch):
        ...
        if task[0] == task[2]:
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                if self.use_sequence_slot_residual:
                    slot_tokens = self.slot_sequence_token(torch.cat(
                        (
                            outgoing_sequence_bank[pair_src],
                            incoming_sequence_bank[pair_dst],
                    ...
                    slot_state = self.slot_sequence_encoder(
                        slot_tokens
                    )[1].squeeze(0)
                    pair_slot_sequence_repr = self.slot_sequence_proj(torch.cat(
                        (
                            pair_sequence_repr,
                            slot_state,
                            pair_sequence_repr * slot_state,
                        ),
                        dim=-1,
                    ))
        ...
        if self.use_sequence_context_residual:
            ...
            if self.use_sequence_slot_residual:
                if pair_slot_sequence_repr is None:
                    pair_slot_sequence_repr = torch.zeros_like(pair_repr)
                slot_sequence_logits = self.slot_sequence_head(
                    pair_slot_sequence_repr[pair_inv][mask]
                )
                pred = pred + (
                    torch.sigmoid(self.slot_sequence_residual_alpha) * slot_sequence_logits
                )
