...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextrecentfuse',
        }
        self.use_chain_context_residual = self.edge_decoding == 'pair_chain_contextresid'
        self.use_recent_context_fusion = self.edge_decoding == 'pair_chain_contextrecentfuse'
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_recent_context_fusion:
                self.context_proj = MLP(dim_in * 3, dim_in,
                                        num_layers=self.head_layers,
                                        bias=True)
                ...
                self.recent_shape_proj = MLP(dim_in * 3, dim_in,
                                             num_layers=self.head_layers,
                                             bias=True)
                self.fusion_proj = MLP(dim_in * 3, dim_in,
                                       num_layers=self.head_layers,
                                       bias=True)
                self.fusion_head = MLP(dim_in * 3, dim_out,
                                       num_layers=self.head_layers,
                                       bias=True)
    ...
    def _pair_chain_head(self, batch):
        ...
        if task[0] == task[2]:
            ...
            if self.use_recent_context_fusion and hasattr(batch[task], 'timestamps'):
        ...
        if self.use_recent_context_fusion:
            ...
            if recent_edge_repr is None:
                recent_edge_repr = torch.zeros_like(edge_repr)
            pair_context_edge_repr = pair_context_repr[pair_inv]
            fused_context_repr = self.fusion_proj(torch.cat(
                (
                    pair_context_edge_repr,
                    recent_edge_repr,
                    pair_context_edge_repr * recent_edge_repr,
                ),
                dim=-1,
            ))
            pred = self.fusion_head(torch.cat(
                (
                    edge_repr[mask],
                    fused_context_repr[mask],
                    edge_repr[mask] * fused_context_repr[mask],
                ),
                dim=-1,
            ))
        else:
