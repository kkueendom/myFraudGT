...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_timeresid',
        }
        self.use_chain_context_residual = self.edge_decoding in {
            'pair_chain_contextresid',
            'pair_chain_timeresid',
        }
        self.use_chain_time_residual = self.edge_decoding == 'pair_chain_timeresid'
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_chain_time_residual:
                self.time_context_proj = MLP(dim_in * 3 + 2, dim_in,
                                             num_layers=self.head_layers,
                                             bias=True)
                self.time_context_head = MLP(dim_in, dim_out,
                                             num_layers=self.head_layers,
                                             bias=True)
                self.time_context_alpha = nn.Parameter(
                    torch.full((1,), math.log(0.10 / 0.90))
                )
        else:
            self.layer_post_mp = MLP(dim_in * 3, dim_out,
    ...
    def _pair_chain_head(self, batch):
        ...
        if self.use_chain_time_residual and hasattr(batch[task], 'timestamps'):
        ...
        if task[0] == task[2]:
            ...
            if self.use_chain_context_residual or self.use_chain_time_residual:
                ...
                focus_interaction = predecessor_focus * successor_focus
                if self.use_chain_context_residual:
                ...
                if self.use_chain_time_residual:
                    if pair_time_mean is not None:
                        predecessor_focus_time_bank = scatter(
                            pair_time_mean * predecessor_focus_weights,
                            pair_dst,
                            dim=0,
                            dim_size=num_nodes,
                            reduce='sum'
                        )
                        successor_focus_time_bank = scatter(
                            pair_time_mean * successor_focus_weights,
                            pair_src,
                            dim=0,
                            dim_size=num_nodes,
                            reduce='sum'
                        )
                        predecessor_gap = torch.log1p(
                            (pair_time_mean - predecessor_focus_time_bank[pair_src]).clamp(min=0)
                        ).unsqueeze(-1)
                        successor_gap = torch.log1p(
                            (successor_focus_time_bank[pair_dst] - pair_time_mean).clamp(min=0)
                        ).unsqueeze(-1)
                    else:
                        predecessor_gap = torch.zeros(
                            (num_pairs, 1), device=edge_repr.device, dtype=edge_repr.dtype
                        )
                        successor_gap = torch.zeros_like(predecessor_gap)
                    # Keep time as a low-init residual branch so the pair-chain path stays intact.
                    pair_time_context_repr = self.time_context_proj(torch.cat(
                        (
                            predecessor_focus,
                            successor_focus,
                            focus_interaction,
        ...
        if self.use_chain_time_residual:
            if pair_time_context_repr is None:
                pair_time_context_repr = torch.zeros_like(pair_repr)
            time_context_logits = self.time_context_head(pair_time_context_repr[pair_inv][mask])
            pred = pred + torch.sigmoid(self.time_context_alpha) * time_context_logits
