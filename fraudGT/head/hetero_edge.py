...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqpairseqbridgebankwindowseqselectxaligndualview',
        }
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_sequence_context_residual:
                ...
                if self.use_sequence_cross_align:
                    ...
                    self.sequence_cross_align_anchor_gate = nn.Linear(dim_in * 4, dim_in)
                    self.sequence_cross_align_anchor_alpha = nn.Parameter(
    ...
    def _cross_align_sequence_banks(self, source_bank, target_bank, pair_repr,
                                    source_partner_bank=None, target_partner_bank=None):
        ...
        if source_partner_bank is not None and target_partner_bank is not None:
            anchor_mask = (
                valid_mask &
                (source_partner_bank >= 0).unsqueeze(2) &
                (target_partner_bank >= 0).unsqueeze(1) &
                (source_partner_bank.unsqueeze(2) == target_partner_bank.unsqueeze(1))
            )
            anchored_source_to_target = self._masked_slot_softmax(
                align_scores, anchor_mask, dim=2
            )
            anchored_target_to_source = self._masked_slot_softmax(
                align_scores, anchor_mask, dim=1
            ).transpose(1, 2)
            anchored_target = torch.matmul(anchored_source_to_target, target_bank)
            anchored_source = torch.matmul(anchored_target_to_source, source_bank)
            anchor_alpha = torch.sigmoid(self.sequence_cross_align_anchor_alpha)

            source_has_anchor = anchor_mask.any(dim=2, keepdim=True).float()
            source_anchor_input = torch.cat(
                (
                    source_bank,
                    aligned_target,
                    anchored_target,
                    aligned_target * anchored_target,
                ),
                dim=-1,
            )
            source_anchor_gate = torch.sigmoid(
                self.sequence_cross_align_anchor_gate(source_anchor_input)
            )
            aligned_target = aligned_target + (
                source_has_anchor *
                anchor_alpha *
                source_anchor_gate *
                (anchored_target - aligned_target)
            )

            target_has_anchor = anchor_mask.transpose(1, 2).any(dim=2, keepdim=True).float()
            target_anchor_input = torch.cat(
                (
                    target_bank,
                    aligned_source,
                    anchored_source,
                    aligned_source * anchored_source,
                ),
                dim=-1,
            )
            target_anchor_gate = torch.sigmoid(
                self.sequence_cross_align_anchor_gate(target_anchor_input)
            )
            aligned_source = aligned_source + (
                target_has_anchor *
                anchor_alpha *
                target_anchor_gate *
                (anchored_source - aligned_source)
            )
        ...
        if task[0] == task[2]:
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                if self.use_sequence_bridge_bank_window:
                    ...
                    if self.use_sequence_cross_align:
                        pair_outgoing_partner_bank = outgoing_partner_bank[pair_src]
                        pair_incoming_partner_bank = incoming_partner_bank[pair_dst]
                        pair_outgoing_sequence_bank, pair_incoming_sequence_bank = (
                            self._cross_align_sequence_banks(
                                pair_outgoing_sequence_bank,
                                pair_incoming_sequence_bank,
                                pair_repr,
                                pair_outgoing_partner_bank,
                                pair_incoming_partner_bank,
                            )
                        )
                ...
                if self.use_sequence_bridge_bank:
                    if outgoing_partner_bank is None or incoming_partner_bank is None:
                        outgoing_partner_bank = self._build_recent_partner_bank(
                            pair_src, pair_dst, pair_timestamps, num_nodes
                        )
                        incoming_partner_bank = self._build_recent_partner_bank(
