...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_history_sequence = self.edge_decoding == 'pair_chain_contextseqpairhist'
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_pair_history_sequence:
                self.pair_history_encoder = nn.GRU(
                    input_size=dim_in,
                    hidden_size=dim_in,
                    batch_first=True,
                )
                self.pair_history_proj = MLP(dim_in * 3, dim_in,
                                             num_layers=self.head_layers,
                                             bias=True)
                self.pair_history_gate = nn.Linear(dim_in * 3, dim_in)
                self.pair_history_residual_alpha = nn.Parameter(
                    torch.full((1,), math.log(0.10 / 0.90))
                )
        else:
            self.layer_post_mp = MLP(dim_in * 3, dim_out,
        ...
        if task[0] == task[2]:
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                if self.use_pair_history_sequence:
                    ...
                    pair_history_gate = torch.sigmoid(self.pair_history_gate(pair_history_input))
                    pair_repr = pair_repr + (
                        torch.sigmoid(self.pair_history_residual_alpha) *
                        pair_history_gate *
                        self.pair_history_proj(pair_history_input)
                    )
