...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqflowresid',
        }
        ...
        self.use_flow_sequence_residual = (
            self.edge_decoding == 'pair_chain_contextseqflowresid'
        )
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_sequence_context_residual:
                ...
                if self.use_flow_sequence_residual:
                    self.predecessor_sequence_encoder = nn.GRU(
                        input_size=dim_in,
                        hidden_size=dim_in,
                        batch_first=True,
                    )
                    self.successor_sequence_encoder = nn.GRU(
                        input_size=dim_in,
                        hidden_size=dim_in,
                        batch_first=True,
                    )
                    self.flow_sequence_proj = MLP(dim_in * 3, dim_in,
                                                  num_layers=self.head_layers,
                                                  bias=True)
                    self.flow_sequence_head = MLP(dim_in, dim_out,
                                                  num_layers=self.head_layers,
                                                  bias=True)
                    self.flow_sequence_residual_alpha = nn.Parameter(
    ...
    def _pair_chain_head(self, batch):
        ...
        if task[0] == task[2]:
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                if self.use_flow_sequence_residual:
                    predecessor_state = self.predecessor_sequence_encoder(
                        incoming_sequence_bank[pair_src]
                    )[1].squeeze(0)
                    successor_state = self.successor_sequence_encoder(
                        outgoing_sequence_bank[pair_dst]
                    )[1].squeeze(0)
                    pair_flow_sequence_repr = self.flow_sequence_proj(torch.cat(
                        (
                            predecessor_state,
                            successor_state,
        ...
        if self.use_sequence_context_residual:
            ...
            if self.use_flow_sequence_residual:
                if pair_flow_sequence_repr is None:
                    pair_flow_sequence_repr = torch.zeros_like(pair_repr)
                flow_sequence_logits = self.flow_sequence_head(
                    pair_flow_sequence_repr[pair_inv][mask]
                )
                pred = pred + (
                    torch.sigmoid(self.flow_sequence_residual_alpha) * flow_sequence_logits
                )
