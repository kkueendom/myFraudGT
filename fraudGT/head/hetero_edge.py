...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqpairseqbridgebanktime',
        }
        ...
        self.use_sequence_bridge_time_bank = (
            self.edge_decoding == 'pair_chain_contextseqpairseqbridgebanktime'
        )
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_sequence_context_residual:
                ...
                if self.use_sequence_bridge_bank:
                    bridge_partner_dim = dim_in * 2 + 2
                    if self.use_sequence_bridge_time_bank:
                        bridge_partner_dim += 2
                    self.bridge_partner_proj = MLP(bridge_partner_dim, dim_in,
                                                   num_layers=self.head_layers,
                                                   bias=True)
    ...
    def _recent_overlap_partner_repr(self, bank_a, bank_b, node_x,
        ...
        if time_a is not None and time_b is not None:
            time_gap = (time_a.unsqueeze(2) - time_b.unsqueeze(1)).abs()
            time_align = torch.exp(
                -time_gap / self.sequence_time_scale.abs().clamp(min=1.0)
            ) * eq.float()
            best_align = time_align.max(dim=2).values * match_any.float()
            mean_align = best_align.sum(dim=1) / overlap_count.clamp(min=1.0)
            weighted_align = (
                best_align * slot_weight.unsqueeze(0)
            ).sum(dim=1)
            partner_feats.extend([
                mean_align.unsqueeze(-1),
                weighted_align.unsqueeze(-1),
            ])
        return self.bridge_partner_proj(torch.cat(partner_feats, dim=-1))
        ...
        if task[0] == task[2]:
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                if self.use_sequence_bridge_bank:
                    ...
                    outgoing_time_bank = None
                    incoming_time_bank = None
                    if self.use_sequence_bridge_time_bank:
                    ...
                    forward_bridge = self._recent_overlap_partner_repr(
                        ...
                        node_x,
                        None if outgoing_time_bank is None else outgoing_time_bank[pair_src],
                        None if incoming_time_bank is None else incoming_time_bank[pair_dst],
                    )
                    cycle_bridge = self._recent_overlap_partner_repr(
                        incoming_partner_bank[pair_src],
                        outgoing_partner_bank[pair_dst],
                        node_x,
                        None if incoming_time_bank is None else incoming_time_bank[pair_src],
                        None if outgoing_time_bank is None else outgoing_time_bank[pair_dst],
                    )
