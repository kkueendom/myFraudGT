...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqpairseqbridgebankwindowseqselectxalignfusion',
        }
        ...
        self.use_sequence_joint_fusion = self.use_sequence_cross_align
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_sequence_context_residual:
                ...
                if self.use_sequence_joint_fusion:
                    self.sequence_joint_fusion = MLP(
                        dim_in * 3, dim_out,
                        num_layers=self.head_layers,
                        bias=True,
                    )
                    self.sequence_joint_alpha = nn.Parameter(
                        torch.full((1,), math.log(0.10 / 0.90))
                    )
        ...
        if self.use_sequence_context_residual:
            ...
            if self.use_sequence_joint_fusion:
                fusion_input = torch.cat(
                    (
                        edge_repr[mask],
                        pair_sequence_repr[pair_inv][mask],
                        edge_repr[mask] * pair_sequence_repr[pair_inv][mask],
                    ),
                    dim=-1,
                )
                pred = pred + (
                    torch.sigmoid(self.sequence_joint_alpha) *
                    self.sequence_joint_fusion(fusion_input)
                )
