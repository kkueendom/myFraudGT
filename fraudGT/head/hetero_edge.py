...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqsetattnresid',
        }
        ...
        self.use_sequence_set_attention_residual = (
            self.edge_decoding == 'pair_chain_contextseqsetattnresid'
        )
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_sequence_context_residual:
                ...
                if self.use_sequence_set_attention_residual:
                    self.sequence_set_query = nn.Linear(dim_in, dim_in)
                    self.sequence_set_key = nn.Linear(dim_in, dim_in)
                    self.sequence_set_value = nn.Linear(dim_in, dim_in)
                    self.sequence_set_out = MLP(dim_in * 3, dim_in,
                                                num_layers=self.head_layers,
                                                bias=True)
                    self.sequence_set_head = MLP(dim_in, dim_out,
                                                 num_layers=self.head_layers,
                                                 bias=True)
                    self.sequence_set_residual_alpha = nn.Parameter(
                        torch.full((1,), math.log(0.05 / 0.95))
                    )
                    self.outgoing_sequence_type = nn.Parameter(torch.zeros(1, 1, dim_in))
                    self.incoming_sequence_type = nn.Parameter(torch.zeros(1, 1, dim_in))
        else:
            self.layer_post_mp = MLP(dim_in * 3, dim_out,
                                     num_layers=self.head_layers,
                                     bias=True)
    ...
    def _pair_chain_head(self, batch):
        ...
        if task[0] == task[2]:
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                if self.use_sequence_set_attention_residual:
                    outgoing_tokens = outgoing_sequence_bank[pair_src] + self.outgoing_sequence_type
                    incoming_tokens = incoming_sequence_bank[pair_dst] + self.incoming_sequence_type
                    sequence_tokens = torch.cat((outgoing_tokens, incoming_tokens), dim=1)
                    query = self.sequence_set_query(pair_sequence_repr).unsqueeze(1)
                    keys = self.sequence_set_key(sequence_tokens)
                    values = self.sequence_set_value(sequence_tokens)
                    attention_scores = torch.matmul(
                        query, keys.transpose(1, 2)
                    ) / math.sqrt(query.size(-1))
                    attention_weights = torch.softmax(attention_scores, dim=-1)
                    sequence_context = torch.matmul(attention_weights, values).squeeze(1)
                    token_summary = sequence_tokens.mean(dim=1)
                    pair_sequence_set_repr = self.sequence_set_out(torch.cat(
                        (
                            pair_sequence_repr,
                            sequence_context,
                            token_summary,
        ...
        if self.use_sequence_context_residual:
            ...
            if self.use_sequence_set_attention_residual:
                if pair_sequence_set_repr is None:
                    pair_sequence_set_repr = torch.zeros_like(pair_repr)
                sequence_set_logits = self.sequence_set_head(
                    pair_sequence_set_repr[pair_inv][mask]
                )
                pred = pred + (
                    torch.sigmoid(self.sequence_set_residual_alpha) * sequence_set_logits
                )
