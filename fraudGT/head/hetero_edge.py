...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_edge_level_sequence = self.edge_decoding == 'pair_chain_contextseqedgeseq'
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_sequence_context_residual:
                ...
                if self.use_edge_level_sequence:
                    self.outgoing_edge_sequence_encoder = nn.GRU(
                        input_size=dim_in,
                        hidden_size=dim_in,
                        batch_first=True,
                    )
                    self.incoming_edge_sequence_encoder = nn.GRU(
                        input_size=dim_in,
                        hidden_size=dim_in,
                        batch_first=True,
                    )
                    self.edge_sequence_proj = MLP(dim_in * 3, dim_in,
                                                  num_layers=self.head_layers,
                                                  bias=True)
                    self.edge_sequence_gate = nn.Linear(dim_in * 3, dim_in)
        else:
            self.layer_post_mp = MLP(dim_in * 3, dim_out,
                                     num_layers=self.head_layers,
                                     bias=True)
    ...
    def _pair_chain_head(self, batch):
        ...
        if task[0] == task[2]:
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                if self.use_edge_level_sequence:
                    outgoing_edge_latest, _ = scatter_max(
                        edge_timestamps, src_nodes, dim=0, dim_size=num_nodes
                    )
                    outgoing_edge_latest = torch.where(
                        torch.isfinite(outgoing_edge_latest),
                        outgoing_edge_latest,
                        torch.zeros_like(outgoing_edge_latest),
                    )
                    incoming_edge_latest, _ = scatter_max(
                        edge_timestamps, dst_nodes, dim=0, dim_size=num_nodes
                    )
                    incoming_edge_latest = torch.where(
                        torch.isfinite(incoming_edge_latest),
                        incoming_edge_latest,
                        torch.zeros_like(incoming_edge_latest),
                    )
                    outgoing_edge_bank = self._build_recent_sequence_bank(
                        edge_repr, src_nodes, edge_timestamps, num_nodes, outgoing_edge_latest
                    )
                    incoming_edge_bank = self._build_recent_sequence_bank(
                        edge_repr, dst_nodes, edge_timestamps, num_nodes, incoming_edge_latest
                    )
                    outgoing_edge_state = self.outgoing_edge_sequence_encoder(
                        outgoing_edge_bank[pair_src]
                    )[1].squeeze(0)
                    incoming_edge_state = self.incoming_edge_sequence_encoder(
                        incoming_edge_bank[pair_dst]
                    )[1].squeeze(0)
                    edge_sequence_repr = self.edge_sequence_proj(torch.cat(
                        (
                            outgoing_edge_state,
                            incoming_edge_state,
                            outgoing_edge_state * incoming_edge_state,
                        ),
                        dim=-1,
                    ))
                    edge_sequence_gate = torch.sigmoid(
                        self.edge_sequence_gate(torch.cat(
                            (outgoing_state, incoming_state, edge_sequence_repr), dim=-1
                        ))
                    )
                    sequence_interaction = (
                        edge_sequence_gate * edge_sequence_repr +
                        (1.0 - edge_sequence_gate) * sequence_interaction
                    )
