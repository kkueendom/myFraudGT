...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqpairseqbridgebankwindowseqselectroleflowboundarysetexclusive',
        ...
        self.use_boundary_set_exclusive = (
            self.edge_decoding ==
            'pair_chain_contextseqpairseqbridgebankwindowseqselectroleflowboundarysetexclusive'
        )
    ...
    def _recent_masked_set_partner_repr(self, bank, node_x, mask, projector):
        valid = (bank >= 0) & mask
        gather_ids = bank.clamp(min=0)
        partner_emb = node_x[gather_ids] * valid.unsqueeze(-1).float()
        count = valid.float().sum(dim=1, keepdim=True)
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                if self.use_sequence_bridge_bank:
                    ...
                    if self.use_terminal_role_flow:
                        ...
                        if self.use_boundary_set_flow:
                            if self.use_boundary_set_exclusive:
                                boundary_overlap = (
                                    incoming_partner_bank[pair_src].unsqueeze(2) ==
                                    outgoing_partner_bank[pair_dst].unsqueeze(1)
                                )
                                ...
                                src_exclusive_mask = (
                                    incoming_partner_bank[pair_src] >= 0
                                ) & ~boundary_overlap.any(dim=2)
                                dst_exclusive_mask = (
                                    outgoing_partner_bank[pair_dst] >= 0
                                ) & ~boundary_overlap.any(dim=1)
                                src_in_set_repr = self._recent_masked_set_partner_repr(
                                    incoming_partner_bank[pair_src],
                                    node_x,
                                    src_exclusive_mask,
                                    self.boundary_partner_set_proj,
                                )
                                dst_out_set_repr = self._recent_masked_set_partner_repr(
                                    outgoing_partner_bank[pair_dst],
                                    node_x,
                                    dst_exclusive_mask,
                                    self.boundary_partner_set_proj,
                                ...
                                src_exclusive_count = (
                                    src_exclusive_mask.float().sum(dim=1, keepdim=True) /
                                    float(self.sequence_len)
                                )
                                dst_exclusive_count = (
                                    dst_exclusive_mask.float().sum(dim=1, keepdim=True) /
                                    float(self.sequence_len)
                                )
                                boundary_set_stats = torch.cat(
                                    (
                                        src_exclusive_count + dst_exclusive_count,
                                        torch.abs(src_exclusive_count - dst_exclusive_count),
                                    ),
                                    dim=-1,
                                )
                            else:
