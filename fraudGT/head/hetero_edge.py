...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqbridgebankhub',
        }
        ...
        self.use_sequence_bridge_bank_hub = (
            self.edge_decoding == 'pair_chain_contextseqbridgebankhub'
        )
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_sequence_context_residual:
                ...
                if self.use_sequence_bridge_bank_hub:
                    self.bridge_partner_proj = MLP(dim_in * 2 + 2, dim_in,
                                                   num_layers=self.head_layers,
                                                   bias=True)
                    self.bridge_flow_hub_proj = MLP(dim_in * 3, dim_in,
                                                    num_layers=self.head_layers,
                                                    bias=True)
                    self.bridge_cycle_hub_proj = MLP(dim_in * 3, dim_in,
                                                     num_layers=self.head_layers,
                                                     bias=True)
                    self.bridge_bank_proj = MLP(dim_in * 5, dim_in,
                                                num_layers=self.head_layers,
                                                bias=True)
        ...
        if task[0] == task[2]:
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                if self.use_sequence_bridge_bank_hub:
                    ...
                    edge_ones = pair_repr.new_ones(num_pairs, 1)
                    out_sum = scatter(node_x[pair_dst], pair_src, dim=0,
                                      dim_size=num_nodes, reduce='sum')
                    out_count = scatter(edge_ones, pair_src, dim=0,
                                        dim_size=num_nodes, reduce='sum').squeeze(-1)
                    in_sum = scatter(node_x[pair_src], pair_dst, dim=0,
                                     dim_size=num_nodes, reduce='sum')
                    in_count = scatter(edge_ones, pair_dst, dim=0,
                                       dim_size=num_nodes, reduce='sum').squeeze(-1)
                    src_out_sum = out_sum[pair_src] - node_x[pair_dst]
                    src_out_count = (out_count[pair_src] - 1.0).clamp(min=0.0)
                    dst_in_sum = in_sum[pair_dst] - node_x[pair_src]
                    dst_in_count = (in_count[pair_dst] - 1.0).clamp(min=0.0)
                    ...
                    flow_hub = self.bridge_flow_hub_proj(torch.cat(
                        (
                            src_out_pool,
                            dst_in_pool,
                    ...
                    cycle_hub = self.bridge_cycle_hub_proj(torch.cat(
                        (
                            src_in_pool,
                            dst_out_pool,
