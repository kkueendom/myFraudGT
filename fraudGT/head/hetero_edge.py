...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqjointresid',
        }
        ...
        self.use_sequence_joint_residual = (
            self.edge_decoding == 'pair_chain_contextseqjointresid'
        )
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_sequence_context_residual:
                ...
                if self.use_sequence_joint_residual:
                    self.joint_sequence_encoder = nn.GRU(
                        input_size=dim_in,
                        hidden_size=dim_in,
                        batch_first=True,
                    )
                    self.joint_sequence_proj = MLP(dim_in * 3, dim_in,
                                                   num_layers=self.head_layers,
                                                   bias=True)
                    self.joint_sequence_head = MLP(dim_in, dim_out,
                                                   num_layers=self.head_layers,
                                                   bias=True)
                    self.joint_sequence_residual_alpha = nn.Parameter(
    ...
    def _build_interleaved_sequence_bank(self, outgoing_bank, incoming_bank):
        num_pairs, seq_len, dim = outgoing_bank.size()
        joint_bank = outgoing_bank.new_zeros((num_pairs, seq_len * 2, dim))
        joint_bank[:, 0::2] = outgoing_bank + self.outgoing_sequence_type
        joint_bank[:, 1::2] = incoming_bank + self.incoming_sequence_type
        return joint_bank
    ...
    def _pair_chain_head(self, batch):
        ...
        if task[0] == task[2]:
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                if self.use_sequence_joint_residual:
                    joint_sequence_bank = self._build_interleaved_sequence_bank(
                        outgoing_sequence_bank[pair_src],
                        incoming_sequence_bank[pair_dst],
                    )
                    joint_state = self.joint_sequence_encoder(
                        joint_sequence_bank
                    )[1].squeeze(0)
                    pair_joint_sequence_repr = self.joint_sequence_proj(torch.cat(
                        (
                            pair_sequence_repr,
                            joint_state,
                            pair_sequence_repr * joint_state,
                        ),
                        dim=-1,
                    ))
        ...
        if self.use_sequence_context_residual:
            ...
            if self.use_sequence_joint_residual:
                if pair_joint_sequence_repr is None:
                    pair_joint_sequence_repr = torch.zeros_like(pair_repr)
                joint_sequence_logits = self.joint_sequence_head(
                    pair_joint_sequence_repr[pair_inv][mask]
                )
                pred = pred + (
                    torch.sigmoid(self.joint_sequence_residual_alpha) * joint_sequence_logits
                )
