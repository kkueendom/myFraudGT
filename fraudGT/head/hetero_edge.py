...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqpairseqbridgebankrecip',
        }
        ...
        self.use_sequence_reciprocal = (
            self.edge_decoding == 'pair_chain_contextseqpairseqbridgebankrecip'
        )
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_sequence_context_residual:
                ...
                if self.use_sequence_bridge_bank:
                    ...
                    if self.use_sequence_reciprocal:
                        self.reciprocal_proj = MLP(dim_in * 3, dim_in,
                                                   num_layers=self.head_layers,
                                                   bias=True)
                        self.reciprocal_gate = nn.Linear(dim_in * 3, dim_in)
                        self.reciprocal_update = MLP(dim_in * 3, dim_in,
                                                     num_layers=self.head_layers,
                                                     bias=True)
                        self.reciprocal_alpha = nn.Parameter(
                            torch.full((1,), math.log(0.10 / 0.90))
                        )
        else:
            self.layer_post_mp = MLP(dim_in * 3, dim_out,
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                if self.use_sequence_bridge_bank:
                    ...
                    if self.use_sequence_reciprocal:
                        ...
                        reciprocal_context = self.reciprocal_proj(torch.cat(
                            (
                                pair_repr,
                                reciprocal_repr,
                        ...
                        reciprocal_input = torch.cat(
                            (
                                reciprocal_context,
                                pair_sequence_repr,
                                reciprocal_context * pair_sequence_repr,
                            ),
                            dim=-1,
                        )
                        reciprocal_gate = torch.sigmoid(self.reciprocal_gate(reciprocal_input))
                        pair_sequence_repr = pair_sequence_repr + (
                            torch.sigmoid(self.reciprocal_alpha) *
                            reciprocal_gate *
                            self.reciprocal_update(reciprocal_input)
                        )
