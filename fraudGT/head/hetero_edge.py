...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqpairseqbridgebankwindowseqselectroleflowboundarylagsubgraphpath',
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_sequence_context_residual:
                ...
                if self.use_sequence_bridge_bank:
                    ...
                    if self.use_sequence_window_path:
                        self.path_sequence_token = nn.Linear(dim_in * 3, dim_in)
                        self.path_sequence_encoder = nn.GRU(
                            input_size=dim_in,
                            hidden_size=dim_in,
                            batch_first=True,
                        )
                        self.path_sequence_proj = MLP(dim_in * 3, dim_in,
                                                      num_layers=self.head_layers,
                                                      bias=True)
                        self.path_sequence_head = MLP(dim_in, dim_out,
                                                      num_layers=self.head_layers,
                                                      bias=True)
                        self.path_sequence_residual_alpha = nn.Parameter(
                            torch.full((1,), math.log(0.05 / 0.95))
                        )
    ...
    def _pair_chain_head(self, batch):
        ...
        if task[0] == task[2]:
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                if self.use_sequence_bridge_bank:
                    ...
                    if self.use_sequence_window_path:
                        pair_center_bank = pair_repr.unsqueeze(1).expand(
                            -1, self.sequence_len, -1
                        )
                        path_tokens = self.path_sequence_token(torch.cat(
                            (
                                outgoing_sequence_bank[pair_src],
                                pair_center_bank,
                                incoming_sequence_bank[pair_dst],
                        ...
                        path_state = self.path_sequence_encoder(
                            path_tokens
                        )[1].squeeze(0)
                        pair_path_sequence_repr = self.path_sequence_proj(torch.cat(
                            (
                                pair_sequence_repr,
                                path_state,
                                pair_sequence_repr * path_state,
                            ),
                            dim=-1,
                        ))
        ...
        if self.use_sequence_context_residual:
            ...
            if self.use_sequence_window_path:
                if pair_path_sequence_repr is None:
                    pair_path_sequence_repr = torch.zeros_like(pair_repr)
                path_sequence_logits = self.path_sequence_head(
                    pair_path_sequence_repr[pair_inv][mask]
                )
                pred = pred + (
                    torch.sigmoid(self.path_sequence_residual_alpha) *
                    path_sequence_logits
                )
        if self.use_terminal_role_flow:
