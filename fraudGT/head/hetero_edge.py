...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqpairseqbridgebankwindowseqselectroleflowboundarysetgate',
                ...
                if self.use_sequence_bridge_bank:
                    ...
                    if self.use_terminal_role_flow:
                        ...
                        if self.use_boundary_set_flow:
                            ...
                            if self.use_boundary_set_gate:
                                self.boundary_set_gate_proj = nn.Linear(
                                    dim_in * 3, dim_in
                                )
                                self.boundary_set_gate_alpha = nn.Parameter(
                                    torch.full((1,), math.log(0.15 / 0.85))
                                )
    ...
    def _pair_chain_head(self, batch):
        ...
        if self.use_boundary_set_flow:
            ...
            boundary_set_repr = pair_boundary_set_repr
            if self.use_boundary_set_gate:
                if pair_boundary_relay_repr is None:
                    pair_boundary_relay_repr = torch.zeros_like(pair_repr)
                gate_input = torch.cat(
                    (
                        boundary_set_repr,
                        pair_boundary_relay_repr,
                        boundary_set_repr * pair_boundary_relay_repr,
                    ),
                    dim=-1,
                )
                gate = torch.sigmoid(self.boundary_set_gate_proj(gate_input))
                gate_alpha = torch.sigmoid(self.boundary_set_gate_alpha)
                boundary_set_repr = boundary_set_repr * (
                    1.0 - gate_alpha + gate_alpha * gate
                )
            boundary_set_logits = self.boundary_set_head(
                boundary_set_repr[pair_inv][mask]
            )
            pred = pred + (
