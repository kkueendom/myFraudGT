...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqpairseqbridgebankrevdiff',
        }
        ...
        self.use_reverse_pair_difference = (
            self.edge_decoding == 'pair_chain_contextseqpairseqbridgebankrevdiff'
        )
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_sequence_context_residual:
                ...
                if self.use_sequence_bridge_bank:
                    ...
                    if self.use_reverse_pair_difference:
                        self.reverse_pair_proj = MLP(dim_in * 3, dim_in,
                                                    num_layers=self.head_layers,
                                                    bias=True)
                        self.reverse_pair_gate = nn.Linear(dim_in * 3, dim_in)
                        self.reverse_pair_update = MLP(dim_in * 3, dim_in,
                                                       num_layers=self.head_layers,
                                                       bias=True)
                        self.reverse_pair_alpha = nn.Parameter(
                            torch.full((1,), math.log(0.10 / 0.90))
                        )
        else:
            self.layer_post_mp = MLP(dim_in * 3, dim_out,
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                if self.use_sequence_bridge_bank:
                    ...
                    if self.use_reverse_pair_difference:
                        ...
                        reverse_indices = self._lookup_pair_indices(pair_keys, reverse_keys)
                        reverse_pair_state = torch.zeros_like(pair_seq_state)
                        valid_reverse = reverse_indices >= 0
                        if valid_reverse.any():
                            reverse_pair_state[valid_reverse] = pair_seq_state[
                                reverse_indices[valid_reverse]
                            ]
                        reverse_delta = pair_seq_state - reverse_pair_state
                        reverse_pair_context = self.reverse_pair_proj(torch.cat(
                            (
                                reverse_delta,
                                pair_seq_state * reverse_pair_state,
                                torch.abs(reverse_delta),
                            ),
                            dim=-1,
                        ))
                        reverse_input = torch.cat(
                            (
                                ...
                                pair_sequence_repr,
                                reverse_pair_context * pair_sequence_repr,
                            ),
                            dim=-1,
                        )
                        reverse_gate = torch.sigmoid(self.reverse_pair_gate(reverse_input))
                        pair_sequence_repr = pair_sequence_repr + (
                            torch.sigmoid(self.reverse_pair_alpha) *
                            reverse_gate *
                            self.reverse_pair_update(reverse_input)
                        )
