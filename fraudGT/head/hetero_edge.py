...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqpairseqbridgebankwindowseqselectroleflowboundarylagsubgraphsupport',
        ...
        self.use_subgraph_support_flow = (
            self.edge_decoding ==
            'pair_chain_contextseqpairseqbridgebankwindowseqselectroleflowboundarylagsubgraphsupport'
        )
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_chain_context_residual:
                ...
                if self.use_subgraph_support_flow:
                    self.subgraph_support_proj = MLP(
                        dim_in * 7, dim_in,
                        num_layers=self.head_layers,
                        bias=True,
                    )
                    self.subgraph_bridge_gate = nn.Linear(dim_in * 4, dim_in)
                    self.subgraph_bridge_update = MLP(
                        dim_in * 4, dim_in,
                        num_layers=self.head_layers,
                        bias=True,
                    )
                    ...
                    self.subgraph_lag_gate = nn.Linear(dim_in * 4, dim_in)
                    self.subgraph_lag_update = MLP(
                        dim_in * 4, dim_in,
                        num_layers=self.head_layers,
                        bias=True,
                    )
                    self.subgraph_lag_alpha = nn.Parameter(
        ...
        if task[0] == task[2]:
            ...
            if self.use_chain_context_residual:
                ...
                if self.use_subgraph_support_flow:
                    ...
                    pair_subgraph_repr = self.subgraph_support_proj(torch.cat(
                        (
                            src_in,
                            src_out,
                            dst_in,
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                if self.use_sequence_bridge_bank:
                    ...
                    if self.use_subgraph_support_flow:
                        ...
                        # Use the subgraph view only to refine the stable bridge context.
                        subgraph_bridge_input = torch.cat(
                            (
                                pair_subgraph_repr,
                                bridge_context,
                                pair_subgraph_repr * bridge_context,
                                torch.abs(pair_subgraph_repr - bridge_context),
                            ),
                            dim=-1,
                        )
                    ...
                    if self.use_terminal_role_flow:
                        ...
                        if self.use_boundary_lag_flow:
                            ...
                            if self.use_subgraph_support_flow:
                                if pair_subgraph_repr is None:
                                    pair_subgraph_repr = torch.zeros_like(pair_repr)
                                subgraph_lag_input = torch.cat(
                                    (
                                        pair_subgraph_repr,
                                        pair_boundary_lag_repr,
                                        pair_subgraph_repr * pair_boundary_lag_repr,
                                        torch.abs(
                                            pair_subgraph_repr - pair_boundary_lag_repr
                                        ),
                                    ),
                                ...
                                subgraph_lag_gate = torch.sigmoid(
                                    self.subgraph_lag_gate(subgraph_lag_input)
                                )
                                pair_boundary_lag_repr = pair_boundary_lag_repr + (
                                    torch.sigmoid(self.subgraph_lag_alpha) *
                                    subgraph_lag_gate *
                                    self.subgraph_lag_update(subgraph_lag_input)
                                )
                    if self.use_sequence_bridge_motif_lite:
                        outgoing_time_bank = self._build_recent_timestamp_bank(
                            pair_src, pair_timestamps, num_nodes
