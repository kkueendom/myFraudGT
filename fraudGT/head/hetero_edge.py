...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqpathbiresid',
        }
        ...
        self.use_sequence_path_bi_residual = (
            self.edge_decoding == 'pair_chain_contextseqpathbiresid'
        )
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_sequence_context_residual:
                ...
                if self.use_sequence_path_bi_residual:
                    self.path_bi_token = nn.Linear(dim_in * 3, dim_in)
                    self.path_bi_encoder = nn.GRU(
                        input_size=dim_in,
                        hidden_size=dim_in // 2,
                        batch_first=True,
                        bidirectional=True,
                    )
                    self.path_bi_proj = MLP(dim_in * 3, dim_in,
                                            num_layers=self.head_layers,
                                            bias=True)
                    self.path_bi_head = MLP(dim_in, dim_out,
                                            num_layers=self.head_layers,
                                            bias=True)
                    self.path_bi_residual_alpha = nn.Parameter(
    ...
    def _pair_chain_head(self, batch):
        ...
        if task[0] == task[2]:
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                if self.use_sequence_path_bi_residual:
                    ...
                    path_tokens = self.path_bi_token(torch.cat(
                        (
                            outgoing_sequence_bank[pair_src],
                            pair_center_bank,
                            incoming_sequence_bank[pair_dst],
                    ...
                    path_hidden = self.path_bi_encoder(path_tokens)[1]
                    path_hidden = path_hidden.transpose(0, 1).reshape(path_tokens.size(0), -1)
                    pair_path_bi_repr = self.path_bi_proj(torch.cat(
                        (
                            pair_sequence_repr,
                            path_hidden,
                            pair_sequence_repr * path_hidden,
                        ),
                        dim=-1,
                    ))
        ...
        if self.use_sequence_context_residual:
            ...
            if self.use_sequence_path_bi_residual:
                if pair_path_bi_repr is None:
                    pair_path_bi_repr = torch.zeros_like(pair_repr)
                path_bi_logits = self.path_bi_head(
                    pair_path_bi_repr[pair_inv][mask]
                )
                pred = pred + (
                    torch.sigmoid(self.path_bi_residual_alpha) * path_bi_logits
                )
