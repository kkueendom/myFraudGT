...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextrecentresid',
        }
        self.use_chain_context_residual = self.edge_decoding in {
            'pair_chain_contextresid',
            'pair_chain_contextrecentresid',
        }
        self.use_recent_context_residual = self.edge_decoding == 'pair_chain_contextrecentresid'
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_recent_context_residual:
                ...
                self.recent_shape_proj = MLP(dim_in * 3, dim_in,
                                             num_layers=self.head_layers,
                                             bias=True)
                self.recent_shape_head = MLP(dim_in, dim_out,
                                             num_layers=self.head_layers,
                                             bias=True)
                self.recent_residual_alpha = nn.Parameter(
                    torch.full((1,), math.log(0.10 / 0.90))
                )
    ...
    def _pair_chain_head(self, batch):
        ...
        if task[0] == task[2]:
            ...
            if self.use_recent_context_residual and hasattr(batch[task], 'timestamps'):
        ...
        if self.use_recent_context_residual:
            ...
            recent_logits = self.recent_shape_head(recent_edge_repr[mask])
            pred = pred + torch.sigmoid(self.recent_residual_alpha) * recent_logits
