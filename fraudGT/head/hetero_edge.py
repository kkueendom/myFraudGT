...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqpairseqbridgebankseqmotif',
        }
        ...
        self.use_sequence_bridge_seqmotif = (
            self.edge_decoding == 'pair_chain_contextseqpairseqbridgebankseqmotif'
        )
            ...
            if self.use_sequence_context_residual:
                ...
                if self.use_sequence_bridge_bank:
                    if self.use_sequence_bridge_seqmotif:
                        self.bridge_token_proj = MLP(dim_in + 2, dim_in,
                                                     num_layers=self.head_layers,
                                                     bias=True)
    ...
    def _recent_overlap_partner_tokens(self, bank_a, bank_b, node_x):
        valid_a = bank_a >= 0
        valid_b = bank_b >= 0
        eq = (
            bank_a.unsqueeze(2) == bank_b.unsqueeze(1)
        ...
        match_any = eq.any(dim=2)
        gather_ids = bank_a.clamp(min=0)
        token_emb = node_x[gather_ids]
        slot_index = torch.arange(
            bank_a.size(1), device=bank_a.device, dtype=torch.float32
        )
        slot_weight = (1.0 / (1.0 + slot_index)).unsqueeze(0).expand_as(bank_a.float())
        ...
        token_repr = self.bridge_token_proj(
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                if self.use_sequence_bridge_bank:
                    ...
                    if self.use_sequence_bridge_seqmotif:
                        ...
                        cycle_tokens = self._recent_overlap_partner_tokens(
                            incoming_partner_bank[pair_src],
                            outgoing_partner_bank[pair_dst],
                            node_x,
                        )
                        forward_bridge = self.bridge_forward_encoder(
                            forward_tokens
                        )[1].squeeze(0)
                        cycle_bridge = self.bridge_cycle_encoder(
                            cycle_tokens
                        )[1].squeeze(0)
