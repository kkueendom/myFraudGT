...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqmultiscale',
        }
        ...
        self.use_multiscale_sequence = (
            self.edge_decoding == 'pair_chain_contextseqmultiscale'
        )
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_sequence_context_residual:
                ...
                if self.use_multiscale_sequence:
                    ...
                    self.multiscale_sequence_proj = MLP(dim_in * 4, dim_in,
                                                        num_layers=self.head_layers,
                                                        bias=True)
                    self.multiscale_sequence_head = MLP(dim_in, dim_out,
                                                        num_layers=self.head_layers,
                                                        bias=True)
                    self.multiscale_residual_alpha = nn.Parameter(
                        torch.full((1,), math.log(0.10 / 0.90))
                    )
    ...
    def _pair_chain_head(self, batch):
        ...
        if task[0] == task[2]:
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                if self.use_multiscale_sequence:
                    ...
                    pair_multiscale_repr = self.multiscale_sequence_proj(torch.cat(
                        (
                            pair_sequence_repr,
                            slow_outgoing_state,
                            slow_incoming_state,
        ...
        if self.use_sequence_context_residual:
            ...
            if self.use_multiscale_sequence:
                if pair_multiscale_repr is None:
                    pair_multiscale_repr = torch.zeros_like(pair_repr)
                multiscale_logits = self.multiscale_sequence_head(
                    pair_multiscale_repr[pair_inv][mask]
                )
                pred = pred + (
                    torch.sigmoid(self.multiscale_residual_alpha) *
                    multiscale_logits
                )
