...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqpairseqbridgebankwindowseqselectroleflowboundarylagsupportmixconsiscalib',
        ...
        self.use_support_calibration_expert = (
            self.edge_decoding ==
            'pair_chain_contextseqpairseqbridgebankwindowseqselectroleflowboundarylagsupportmixconsiscalib'
        )
            ...
            if self.use_sequence_context_residual:
                ...
                if self.use_sequence_bridge_bank:
                    ...
                    if self.use_support_conditioned_mixture:
                        ...
                        if self.use_support_calibration_expert:
                            self.support_calibration_gate = MLP(
                                dim_in + self.support_feature_dim, 1,
                                num_layers=self.head_layers,
                                bias=True,
                            )
                            self.support_calibration_bias = nn.Parameter(
                                torch.tensor(math.log(0.12 / 0.88))
                            )
                            self.support_calibration_head = MLP(
                                dim_in + self.support_feature_dim, 1,
                                num_layers=self.head_layers,
                                bias=True,
                            )
                            self.support_calibration_alpha = nn.Parameter(
                                torch.full((1,), math.log(0.08 / 0.92))
                            )
    ...
    def _pair_chain_head(self, batch):
        ...
        pair_calibration_repr = None
        pair_calibration_support = None
        ...
        if task[0] == task[2]:
            ...
            if self.use_support_conditioned_mixture:
                ...
                if self.use_support_calibration_expert:
                    pair_calibration_repr = torch.cat(
                        (pair_repr, pair_support_features), dim=-1
                    )
                    pair_calibration_support = torch.sigmoid(
                        self.support_calibration_bias +
                        self.support_calibration_gate(pair_calibration_repr)
                    )
                if self.use_support_prototype_expert:
        ...
        if self.use_support_calibration_expert:
            if pair_calibration_repr is None:
                pair_calibration_repr = torch.cat(
                    (
                        pair_repr,
                        pair_repr.new_zeros((pair_repr.size(0), self.support_feature_dim)),
                    ),
                    dim=-1,
                )
            calibration_delta = self.support_calibration_head(
                pair_calibration_repr[pair_inv][mask]
            )
            if pair_calibration_support is not None:
                calibration_delta = (
                    pair_calibration_support[pair_inv][mask] * calibration_delta
                )
            if pred.size(-1) == 1:
                calibration_logits = calibration_delta
            else:
            ...
            pred = pred + (
                torch.sigmoid(self.support_calibration_alpha) *
                calibration_logits
            )
