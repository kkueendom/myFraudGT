...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextrecentmix',
        }
        self.use_chain_context_residual = self.edge_decoding in {
            'pair_chain_contextresid',
            'pair_chain_contextrecentmix',
        }
        self.use_recent_context_mix = self.edge_decoding == 'pair_chain_contextrecentmix'
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_recent_context_mix:
                ...
                self.recent_pair_proj = MLP(dim_in * 3, dim_in,
                                            num_layers=self.head_layers,
                                            bias=True)
                self.context_mix_proj = MLP(dim_in * 3, dim_in,
                                            num_layers=self.head_layers,
                                            bias=True)
    ...
    def _pair_chain_head(self, batch):
        ...
        if task[0] == task[2]:
            ...
            if self.use_recent_context_mix and hasattr(batch[task], 'timestamps'):
                ...
                pair_context_repr = self.context_mix_proj(torch.cat(
                    (
                        pair_context_repr,
                        pair_recent_repr,
                        pair_context_repr * pair_recent_repr,
                    ),
                    dim=-1,
                ))
