...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqpathshaperesid',
        }
        ...
        self.use_sequence_path_shape_residual = (
            self.edge_decoding == 'pair_chain_contextseqpathshaperesid'
        )
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_sequence_context_residual:
                ...
                if self.use_sequence_path_shape_residual:
                    self.path_shape_token = nn.Linear(dim_in * 6, dim_in)
                    self.path_shape_encoder = nn.GRU(
                        input_size=dim_in,
                        hidden_size=dim_in,
                        batch_first=True,
                    )
                    self.path_shape_proj = MLP(dim_in * 3, dim_in,
                                               num_layers=self.head_layers,
                                               bias=True)
                    self.path_shape_head = MLP(dim_in, dim_out,
                                               num_layers=self.head_layers,
                                               bias=True)
                    self.path_shape_residual_alpha = nn.Parameter(
    ...
    def _pair_chain_head(self, batch):
        ...
        if task[0] == task[2]:
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                if self.use_sequence_path_shape_residual:
                    ...
                    outgoing_pair = outgoing_sequence_bank[pair_src]
                    incoming_pair = incoming_sequence_bank[pair_dst]
                    path_shape_tokens = self.path_shape_token(torch.cat(
                        (
                            outgoing_pair,
                            pair_center_bank,
                            incoming_pair,
                            outgoing_pair * pair_center_bank,
                            pair_center_bank * incoming_pair,
                            outgoing_pair - incoming_pair,
                        ),
                        dim=-1,
                    ))
                    path_shape_state = self.path_shape_encoder(
                        path_shape_tokens
                    )[1].squeeze(0)
                    pair_path_shape_repr = self.path_shape_proj(torch.cat(
                        (
                            pair_sequence_repr,
                            path_shape_state,
                            pair_sequence_repr * path_shape_state,
                        ),
                        dim=-1,
                    ))
        ...
        if self.use_sequence_context_residual:
            ...
            if self.use_sequence_path_shape_residual:
                if pair_path_shape_repr is None:
                    pair_path_shape_repr = torch.zeros_like(pair_repr)
                path_shape_logits = self.path_shape_head(
                    pair_path_shape_repr[pair_inv][mask]
                )
                pred = pred + (
                    torch.sigmoid(self.path_shape_residual_alpha) * path_shape_logits
                )
