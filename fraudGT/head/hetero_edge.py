...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqpairseqbridgebankpeer',
        }
        ...
        self.use_sequence_bridge_peer = (
            self.edge_decoding == 'pair_chain_contextseqpairseqbridgebankpeer'
        )
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_sequence_context_residual:
                ...
                if self.use_sequence_bridge_bank:
                    ...
                    if self.use_sequence_bridge_peer:
                        self.source_peer_score = nn.Linear(dim_in, 1)
                        self.dest_peer_score = nn.Linear(dim_in, 1)
                        self.peer_bank_proj = MLP(dim_in * 3, dim_in,
                                                  num_layers=self.head_layers,
                                                  bias=True)
                        self.peer_bank_gate = nn.Linear(dim_in * 3, dim_in)
                        self.peer_bank_update = MLP(dim_in * 3, dim_in,
                                                    num_layers=self.head_layers,
                                                    bias=True)
                        self.peer_bank_alpha = nn.Parameter(
                            torch.full((1,), math.log(0.10 / 0.90))
                        )
        else:
            self.layer_post_mp = MLP(dim_in * 3, dim_out,
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                if self.use_sequence_bridge_bank:
                    ...
                    if self.use_sequence_bridge_peer:
                        ...
                        peer_context = self.peer_bank_proj(torch.cat(
                            (
                                source_peer_context,
                                dest_peer_context,
                                source_peer_context * dest_peer_context,
                            ),
                            dim=-1,
                        ))
                        peer_input = torch.cat(
                            (
                                peer_context,
                                pair_sequence_repr,
                                peer_context * pair_sequence_repr,
                            ),
                            dim=-1,
                        )
                        peer_gate = torch.sigmoid(self.peer_bank_gate(peer_input))
                        pair_sequence_repr = pair_sequence_repr + (
                            torch.sigmoid(self.peer_bank_alpha) *
                            peer_gate *
                            self.peer_bank_update(peer_input)
                        )
