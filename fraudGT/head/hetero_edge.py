...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqpairseqmotifbank',
        }
        ...
        self.use_motif_bank = (
            self.edge_decoding == 'pair_chain_contextseqpairseqmotifbank'
        )
            ...
            if self.use_sequence_context_residual:
                ...
                if self.use_sequence_bridge_bank:
                    self.bridge_partner_proj = MLP(dim_in * 2 + 2, dim_in,
                                                   num_layers=self.head_layers,
                                                   bias=True)
                    if self.use_motif_bank:
                        self.motif_relation_emb = nn.Parameter(
                            torch.randn(4, dim_in) * 0.02
                        )
                        self.motif_sequence_encoder = nn.GRU(
                            input_size=dim_in,
                            hidden_size=dim_in,
                            batch_first=True,
                        )
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                if self.use_sequence_bridge_bank:
                    ...
                    if self.use_motif_bank:
                        ...
                        motif_tokens = motif_tokens + self.motif_relation_emb.unsqueeze(0)
                        motif_state = self.motif_sequence_encoder(
                            motif_tokens
                        )[1].squeeze(0)
                        motif_mean = motif_tokens.mean(dim=1)
                        bridge_context = self.bridge_bank_proj(torch.cat(
                            (motif_state, motif_mean),
                            dim=-1,
                        ))
                    else:
                        bridge_context = self.bridge_bank_proj(torch.cat(
