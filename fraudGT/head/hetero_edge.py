...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqpairseqbridgebankwindowseqselectlinegraph',
        }
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_edge_dual_line_graph:
                self.prev_line_stat_proj = MLP(dim_in * 3, dim_in,
                                              num_layers=self.head_layers,
                                              bias=True)
                self.next_line_stat_proj = MLP(dim_in * 3, dim_in,
                                              num_layers=self.head_layers,
                                              bias=True)
                self.prev_line_context_proj = MLP(dim_in * 3, dim_in,
                                                 num_layers=self.head_layers,
                                                 bias=True)
                self.next_line_context_proj = MLP(dim_in * 3, dim_in,
                                                 num_layers=self.head_layers,
                                                 bias=True)
                self.line_graph_gate = nn.Linear(dim_in * 3, dim_in)
                self.line_graph_update = MLP(dim_in * 3, dim_in,
                                             num_layers=self.head_layers,
                                             bias=True)
                self.line_graph_alpha = nn.Parameter(
                    torch.full((1,), math.log(0.10 / 0.90))
                )
    ...
    def _pair_chain_head(self, batch):
        ...
        if task[0] == task[2] and self.use_edge_dual_line_graph:
            num_nodes = batch[task[0]].x.size(0)
            predecessor_mean = scatter(
                edge_repr, dst_nodes, dim=0, dim_size=num_nodes, reduce='mean'
            )
            predecessor_max, _ = scatter_max(
                edge_repr, dst_nodes, dim=0, dim_size=num_nodes
            )
            predecessor_max = torch.where(
                torch.isfinite(predecessor_max),
                predecessor_max,
                torch.zeros_like(predecessor_max),
            )
            successor_mean = scatter(
                edge_repr, src_nodes, dim=0, dim_size=num_nodes, reduce='mean'
            )
            successor_max, _ = scatter_max(
                edge_repr, src_nodes, dim=0, dim_size=num_nodes
            )
            successor_max = torch.where(
                torch.isfinite(successor_max),
                successor_max,
                torch.zeros_like(successor_max),
            )
            predecessor_focus_bank = self._build_focus_bank(
                edge_repr, dst_nodes, num_nodes
            )
            successor_focus_bank = self._build_focus_bank(
                edge_repr, src_nodes, num_nodes
            )
            predecessor_stat = self.prev_line_stat_proj(torch.cat(
                (
                    predecessor_mean[src_nodes],
                    predecessor_max[src_nodes],
                    predecessor_max[src_nodes] - predecessor_mean[src_nodes],
                ),
                dim=-1,
            ))
            successor_stat = self.next_line_stat_proj(torch.cat(
                (
                    successor_mean[dst_nodes],
                    successor_max[dst_nodes],
                    successor_max[dst_nodes] - successor_mean[dst_nodes],
                ),
                dim=-1,
            ))
            predecessor_focus = predecessor_focus_bank[src_nodes]
            successor_focus = successor_focus_bank[dst_nodes]
            predecessor_context = self.prev_line_context_proj(torch.cat(
                (
                    predecessor_stat,
                    predecessor_focus,
                    predecessor_stat * predecessor_focus,
                ),
                dim=-1,
            ))
            successor_context = self.next_line_context_proj(torch.cat(
                (
                    successor_stat,
                    successor_focus,
                    successor_stat * successor_focus,
                ),
                dim=-1,
            ))
            ...
            line_gate = torch.sigmoid(self.line_graph_gate(line_input))
            edge_repr = edge_repr + (
                torch.sigmoid(self.line_graph_alpha) *
                line_gate *
                self.line_graph_update(line_input)
            )
