...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqbridgebankstats',
        }
        ...
        self.use_sequence_bridge_bank_stats = (
            self.edge_decoding == 'pair_chain_contextseqbridgebankstats'
        )
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_sequence_context_residual:
                ...
                if self.use_sequence_bridge_bank_stats:
                    ...
                    self.bridge_stats_proj = MLP(10, dim_in,
                                                 num_layers=self.head_layers,
                                                 bias=True)
                    self.bridge_bank_proj = MLP(dim_in * 4, dim_in,
                                                num_layers=self.head_layers,
                                                bias=True)
        ...
        if task[0] == task[2]:
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                if self.use_sequence_bridge_bank_stats:
                    ...
                    valid_src_out = src_out_partners >= 0
                    valid_dst_in = dst_in_sources >= 0
                    valid_src_in = src_in_sources >= 0
                    valid_dst_out = dst_out_partners >= 0
                    forward_match = (
                        src_out_partners.unsqueeze(2) == dst_in_sources.unsqueeze(1)
                    ) & valid_src_out.unsqueeze(2) & valid_dst_in.unsqueeze(1)
                    cycle_match = (
                        src_in_sources.unsqueeze(2) == dst_out_partners.unsqueeze(1)
                    ) & valid_src_in.unsqueeze(2) & valid_dst_out.unsqueeze(1)
                    forward_count = (
                        forward_match.any(dim=2).float() * valid_src_out.float()
                    ).sum(dim=1)
                    cycle_count = (
                        cycle_match.any(dim=2).float() * valid_src_in.float()
                    ).sum(dim=1)
                    slot_index = torch.arange(
                        src_out_partners.size(1), device=pair_repr.device, dtype=torch.float32
                    )
                    slot_weight = 1.0 / (1.0 + slot_index)
                    forward_weight = (
                        forward_match.any(dim=2).float() * slot_weight.unsqueeze(0)
                    ).sum(dim=1)
                    cycle_weight = (
                        cycle_match.any(dim=2).float() * slot_weight.unsqueeze(0)
                    ).sum(dim=1)
                    shared_out_match = (
                        src_out_partners.unsqueeze(2) == dst_out_partners.unsqueeze(1)
                    ) & valid_src_out.unsqueeze(2) & valid_dst_out.unsqueeze(1)
                    shared_in_match = (
                        src_in_sources.unsqueeze(2) == dst_in_sources.unsqueeze(1)
                    ) & valid_src_in.unsqueeze(2) & valid_dst_in.unsqueeze(1)
                    shared_out_count = (
                        shared_out_match.any(dim=2).float() * valid_src_out.float()
                    ).sum(dim=1)
                    shared_in_count = (
                        shared_in_match.any(dim=2).float() * valid_src_in.float()
                    ).sum(dim=1)
                    shared_out_weight = (
                        shared_out_match.any(dim=2).float() * slot_weight.unsqueeze(0)
                    ).sum(dim=1)
                    shared_in_weight = (
                        shared_in_match.any(dim=2).float() * slot_weight.unsqueeze(0)
                    ).sum(dim=1)
                    reciprocal_recent = (
                        (dst_out_partners == pair_src.unsqueeze(-1)).any(dim=1).float() +
                        (src_in_sources == pair_dst.unsqueeze(-1)).any(dim=1).float()
                    )
                    bridge_stats = self.bridge_stats_proj(torch.stack(
                        (
                            forward_count,
                            cycle_count,
                            shared_out_count,
