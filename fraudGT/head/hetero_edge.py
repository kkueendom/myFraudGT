...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqpairseqbridgebanklineseq',
        }
        ...
        self.use_edge_line_sequence = (
            self.edge_decoding == 'pair_chain_contextseqpairseqbridgebanklineseq'
        )
        ...
        if self.use_pair_chain_head:
            self.edge_proj = MLP(dim_in * 3, dim_in,
                                 ...
                                 bias=True)
            if self.use_edge_line_sequence:
                self.line_sequence_len = 4
                self.predecessor_edge_encoder = nn.GRU(
                    input_size=dim_in,
                    hidden_size=dim_in,
                    batch_first=True,
                )
                self.successor_edge_encoder = nn.GRU(
                    input_size=dim_in,
                    hidden_size=dim_in,
                    batch_first=True,
                )
                self.line_sequence_proj = MLP(dim_in * 3, dim_in,
                                              num_layers=self.head_layers,
                                              bias=True)
                self.line_sequence_gate = nn.Linear(dim_in * 3, dim_in)
                self.line_sequence_update = MLP(dim_in * 3, dim_in,
                                                num_layers=self.head_layers,
                                                bias=True)
                self.line_sequence_alpha = nn.Parameter(
    ...
    def _pair_chain_head(self, batch):
        ...
        if hasattr(batch[task], 'timestamps'):
            edge_timestamps = batch[task].timestamps.to(edge_repr.device).float().view(-1)

        if self.use_edge_line_sequence and task[0] == task[2] and edge_timestamps is not None:
            ...
            predecessor_edge_bank = self._build_recent_sequence_bank(
                edge_repr, dst_nodes, edge_timestamps, num_nodes, predecessor_latest
            )
            successor_edge_bank = self._build_recent_sequence_bank(
                edge_repr, src_nodes, edge_timestamps, num_nodes, successor_latest
            )
            predecessor_edge_state = self.predecessor_edge_encoder(
                predecessor_edge_bank[src_nodes]
            )[1].squeeze(0)
            successor_edge_state = self.successor_edge_encoder(
                successor_edge_bank[dst_nodes]
            )[1].squeeze(0)
            line_context = self.line_sequence_proj(torch.cat(
                (
                    predecessor_edge_state,
                    successor_edge_state,
                    predecessor_edge_state * successor_edge_state,
                ),
                dim=-1,
            ))
            line_input = torch.cat(
                (
                    line_context,
                    edge_repr,
                    line_context * edge_repr,
                ),
                dim=-1,
            )
            line_gate = torch.sigmoid(self.line_sequence_gate(line_input))
            edge_repr = edge_repr + (
                torch.sigmoid(self.line_sequence_alpha) *
                line_gate *
                self.line_sequence_update(line_input)
            )
        ...
        pair_seq_state = None
        if self.use_pair_internal_sequence and edge_timestamps is not None:
        ...
        if task[0] == task[2]:
            ...
            if self.use_sequence_context_residual and edge_timestamps is not None:
