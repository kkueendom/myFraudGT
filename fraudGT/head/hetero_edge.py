...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqotherqueryresid',
            'pair_chain_contextseqotherqueryrecipresid',
        }
        ...
        self.use_other_sequence_query_residual = (
        ...
        self.use_reciprocal_pair_residual = (
            self.edge_decoding == 'pair_chain_contextseqotherqueryrecipresid'
        )
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_sequence_context_residual:
                ...
                if self.use_other_sequence_query_residual:
                    self.sequence_query = nn.Linear(dim_in, dim_in)
                    self.sequence_key = nn.Linear(dim_in, dim_in)
                    self.sequence_value = nn.Linear(dim_in, dim_in)
                    self.sequence_query_proj = MLP(dim_in * 5, dim_in,
                                                   num_layers=self.head_layers,
                                                   bias=True)
                    self.sequence_query_head = MLP(dim_in, dim_out,
                                                   num_layers=self.head_layers,
                                                   bias=True)
                    self.sequence_query_residual_alpha = nn.Parameter(
                ...
                if self.use_reciprocal_pair_residual:
                    self.reciprocal_proj = MLP(dim_in * 4, dim_in,
                                               num_layers=self.head_layers,
                                               bias=True)
                    self.reciprocal_head = MLP(dim_in, dim_out,
                                               num_layers=self.head_layers,
                                               bias=True)
                    self.reciprocal_residual_alpha = nn.Parameter(
                        torch.full((1,), math.log(0.08 / 0.92))
                    )
        else:
            self.layer_post_mp = MLP(dim_in * 3, dim_out,
    ...
    def _attend_sequence_bank(self, query_repr, sequence_bank, sequence_indices=None,
                              exclude_index=None):
        query = self.sequence_query(query_repr).unsqueeze(1)
        keys = self.sequence_key(sequence_bank)
        values = self.sequence_value(sequence_bank)
        scores = (query * keys).sum(dim=-1) / math.sqrt(query_repr.size(-1))
        valid_mask = sequence_bank.abs().sum(dim=-1) > 0
        if sequence_indices is not None and exclude_index is not None:
            valid_mask = valid_mask & (sequence_indices != exclude_index.unsqueeze(-1))
    ...
    def _pair_chain_head(self, batch):
        ...
        if task[0] == task[2]:
            ...
            if self.use_reciprocal_pair_residual:
                ...
                reciprocal_pair_repr[reciprocal_valid] = pair_repr[safe_reverse_pos[reciprocal_valid]]
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                if self.use_other_sequence_query_residual:
                    outgoing_attn = self._attend_sequence_bank(
                        pair_repr,
                        outgoing_sequence_bank[pair_src],
                    ...
                    pair_sequence_query_repr = self.sequence_query_proj(torch.cat(
                        (
                            outgoing_attn,
                            incoming_attn,
                            pair_repr - outgoing_attn,
                            pair_repr - incoming_attn,
                            outgoing_attn * incoming_attn,
                        ),
                        dim=-1,
                    ))
        ...
        if self.use_other_sequence_query_residual:
            if pair_sequence_query_repr is None:
                pair_sequence_query_repr = torch.zeros_like(pair_repr)
            sequence_query_logits = self.sequence_query_head(
                pair_sequence_query_repr[pair_inv][mask]
            )
            pred = pred + (
                torch.sigmoid(self.sequence_query_residual_alpha) *
                sequence_query_logits
            )
        if self.use_reciprocal_pair_residual:
            if pair_reciprocal_repr is None:
                pair_reciprocal_repr = torch.zeros_like(pair_repr)
            reciprocal_logits = self.reciprocal_head(
                pair_reciprocal_repr[pair_inv][mask]
            )
            pred = pred + (
                torch.sigmoid(self.reciprocal_residual_alpha) *
                reciprocal_logits
            )
