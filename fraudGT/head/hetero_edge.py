...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqpairseqbridgebankshared',
        }
        ...
        self.use_sequence_shared_bank = (
            self.edge_decoding == 'pair_chain_contextseqpairseqbridgebankshared'
        )
        ...
        if self.use_pair_chain_head:
            ...
            if self.use_sequence_context_residual:
                ...
                if self.use_sequence_shared_bank:
                    self.shared_bank_proj = MLP(dim_in * 3, dim_in,
                                                num_layers=self.head_layers,
                                                bias=True)
                    self.shared_bank_gate = nn.Linear(dim_in * 3, dim_in)
                    self.shared_bank_update = MLP(dim_in * 3, dim_in,
                                                  num_layers=self.head_layers,
                                                  bias=True)
                    self.shared_bank_alpha = nn.Parameter(
        ...
        if task[0] == task[2]:
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                if self.use_sequence_bridge_bank:
                    ...
                    if self.use_sequence_shared_bank:
                        shared_out = self._recent_overlap_partner_repr(
                            outgoing_partner_bank[pair_src],
                            outgoing_partner_bank[pair_dst],
                            node_x,
                        )
                        shared_in = self._recent_overlap_partner_repr(
                            incoming_partner_bank[pair_src],
                            incoming_partner_bank[pair_dst],
                            node_x,
                        )
                        shared_context = self.shared_bank_proj(torch.cat(
                            (
                                shared_out,
                                shared_in,
                        ...
                        shared_input = torch.cat(
                            (
                                shared_context,
                                pair_sequence_repr,
                                shared_context * pair_sequence_repr,
                            ),
                            dim=-1,
                        )
                        shared_gate = torch.sigmoid(self.shared_bank_gate(shared_input))
                        pair_sequence_repr = pair_sequence_repr + (
                            torch.sigmoid(self.shared_bank_alpha) *
                            shared_gate *
                            self.shared_bank_update(shared_input)
                        )
