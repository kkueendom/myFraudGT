...
class HeteroGNNEdgeHead(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dataset):
        ...
        self.use_pair_chain_head = self.edge_decoding in {
            ...
            'pair_chain_contextseqpairseqbridgebankmotifstats',
        }
        ...
        self.use_sequence_bridge_motifstats = (
            self.edge_decoding == 'pair_chain_contextseqpairseqbridgebankmotifstats'
        )
            ...
            if self.use_sequence_context_residual:
                ...
                if self.use_sequence_bridge_bank:
                    ...
                    self.bridge_bank_proj = MLP(dim_in * 3, dim_in,
                                                num_layers=self.head_layers,
                                                bias=True)
                    if self.use_sequence_bridge_motifstats:
                        self.bridge_stats_proj = MLP(8, dim_in,
                                                     num_layers=self.head_layers,
                                                     bias=True)
    ...
    def _recent_overlap_partner_stats(self, bank_a, bank_b):
        ...
        return torch.stack((overlap_count, weighted_overlap), dim=-1)
        ...
        if task[0] == task[2]:
            ...
            if self.use_sequence_context_residual and hasattr(batch[task], 'timestamps'):
                ...
                if self.use_sequence_bridge_bank:
                    ...
                    if self.use_sequence_bridge_motifstats:
                        forward_stats = self._recent_overlap_partner_stats(
                            outgoing_partner_bank[pair_src],
                            incoming_partner_bank[pair_dst],
                        )
                        cycle_stats = self._recent_overlap_partner_stats(
                            incoming_partner_bank[pair_src],
                            outgoing_partner_bank[pair_dst],
                        )
                        shared_out_stats = self._recent_overlap_partner_stats(
                            outgoing_partner_bank[pair_src],
                            outgoing_partner_bank[pair_dst],
                        )
                        shared_in_stats = self._recent_overlap_partner_stats(
                            incoming_partner_bank[pair_src],
                            incoming_partner_bank[pair_dst],
                        )
                        bridge_context = bridge_context + self.bridge_stats_proj(
                            torch.cat(
                                (
                                    forward_stats,
                                    cycle_stats,
