#!/usr/bin/env python3
"""GPU queue for the matched 120-epoch DMPRD P0 quick screen."""
...
OUT_DIR = Path(os.environ.get(
    "DMPRD_P0_OUT_DIR", str(REPO / "results" / "dmprd_p0_quick120")))
EVENTS = REPO / ".dmprd_p0_quick120_queue.events"
ACTIVE_DIR = REPO / ".dmprd_p0_quick120_active"
FAILED_DIR = REPO / ".dmprd_p0_quick120_failed"
...
# Start the expensive Large-LI and Medium-HI pairs first. Small-LI controls
# fill the next GPUs, and the one-component diagnostic runs when a GPU frees.
TASKS = [
    ("Large-LI", "a2_control", 44),
    ("Large-LI", "p0_full", 44),
    ("Medium-HI", "a2_control", 42),
    ("Medium-HI", "p0_full", 42),
    ("Small-LI", "p0_full", 42),
    ("Small-LI", "a2_control", 42),
    ("Small-LI", "p0_supportvar", 42),
]
...
def run_stem(dataset, variant, seed):
    return f"AML-{dataset}-DMPRDP0Quick120-{variant}-Seed{seed}"
...
def launch(dataset, variant_name, seed, gpu):
    ...
    stdout_path = REPO / f".dmprd_p0_quick120_{run_name}_gpu{gpu}.log"
    cmd = [
        PYTHON, "-m", "fraudGT.main",
        "--cfg", f"configs/evidence_gate_v4/AML-{dataset}.yaml",
        "--repeat", "1", "--gpu", "0",
        "out_dir", str(OUT_DIR),
        "name_tag", f"DMPRDP0Quick120-{variant_name}-Seed{seed}",
        "seed", str(seed),
...
def main():
    log(
        f"DMPRD P0 quick queue start max_epoch={MAX_EPOCH} "
    ...
    while True:
        ...
        if not pending and not markers:
            ...
            log(f"DMPRD P0 quick queue finished failures={failures}")
