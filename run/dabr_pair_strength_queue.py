#!/usr/bin/env python3
"""Two-dataset, 500-epoch queue for one DABR route strength.
...
guard. Separate processes may run different pre-registered strengths.
"""
...
REPO = Path(os.environ.get(
    "DABR_REPO", str(Path(__file__).resolve().parents[1]))).resolve()
PYTHON = os.environ.get(
    "FRAUDGT_PYTHON", "/d/miniconda3/envs/fraudGT/bin/python3.9")
MAX_EPOCH = int(os.environ.get("DABR_MAX_EPOCH", "500"))
DONE_EPOCH = MAX_EPOCH - 1
POLL_SECONDS = int(os.environ.get("DABR_POLL_SECONDS", "1200"))
LAUNCH_SETTLE_SECONDS = int(os.environ.get(
    "DABR_LAUNCH_SETTLE_SECONDS", "20"))
MIN_FREE_MIB = int(os.environ.get("DABR_MIN_FREE_MIB", "9000"))
MAX_UTIL = int(os.environ.get("DABR_MAX_UTIL", "15"))
GPU_ALLOWLIST = tuple(int(item) for item in os.environ.get(
    "DABR_GPU_ALLOWLIST", "1,2,3,4,5,6").split(",") if item.strip())
IGNORE_PROCESS_PATTERNS = tuple(item.strip().lower() for item in os.environ.get(
    "DABR_IGNORE_PROCESS_PATTERNS", "ocr").split(",") if item.strip())
EXPECTED_BRANCH = os.environ.get(
    "DABR_EXPECTED_BRANCH", "feature/dabr-decoupled-ternary-router")
OUT_BASE = Path(os.environ.get(
    "DABR_OUT_BASE", str(REPO / "results"))).resolve()
RUNTIME_BASE = Path(os.environ.get(
    "DABR_RUNTIME_BASE", str(REPO.parent / "FraudGT_dabr_runtime"))).resolve()
ROUTE_STRENGTH = float(os.environ.get("DABR_ROUTE_STRENGTH", "0.25"))
if not 0.0 < ROUTE_STRENGTH <= 1.0:
    raise ValueError("DABR_ROUTE_STRENGTH must be in (0, 1]")
STRENGTH_TAG = f"s{int(round(100 * ROUTE_STRENGTH)):02d}"
OUT_DIR = None
EVENTS = None
ACTIVE_DIR = None
FAILED_DIR = None
...
def configure_paths(git_sha):
    """Bind every artifact path to strength and source commit."""
    global OUT_DIR, EVENTS, ACTIVE_DIR, FAILED_DIR
    namespace = f"dabr_{STRENGTH_TAG}_{git_sha}_formal500"
    OUT_DIR = OUT_BASE / namespace
    RUNTIME_BASE.mkdir(parents=True, exist_ok=True)
    EVENTS = RUNTIME_BASE / f".{namespace}_queue.events"
...
def run_stem(dataset, seed, git_sha):
    return (
        f"AML-{dataset}-DABR{STRENGTH_TAG}Formal500-full-"
        f"Seed{seed}-{git_sha}")
...
def launch(dataset, seed, gpu, git_sha):
    ...
    stdout_path = (
        RUNTIME_BASE /
        f".dabr_{STRENGTH_TAG}_{git_sha}_{run_name}_gpu{gpu}.log")
    cmd = [
        ...
        "out_dir", str(OUT_DIR),
        "name_tag",
        f"DABR{STRENGTH_TAG}Formal500-full-Seed{seed}-{git_sha}",
        "seed", str(seed),
        ...
        "model.dabr_aux_weight", "0.05",
        "model.dabr_aux_start_epoch", "10",
        "model.dabr_aux_end_epoch", "150",
        "model.dabr_gap_floor", "0.0001",
        "model.dabr_gap_threshold", "0.005",
        "model.dabr_router_hidden", "32",
        "model.dabr_route_strength", str(ROUTE_STRENGTH),
    ]
    env = os.environ.copy()
...
def main():
    ...
    log(
        f"DABR pair queue start strength={ROUTE_STRENGTH:.2f} "
        f"tag={STRENGTH_TAG} commit={git_sha} "
    ...
    while True:
        ...
        if not pending and not markers:
            ...
            log(
                f"DABR pair queue finished tag={STRENGTH_TAG} "
                f"failures={failures}")
