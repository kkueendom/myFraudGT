#!/usr/bin/env python3
"""GPU queue for the 120-epoch DMPRD go/no-go screen.
...
MAX_EPOCH = int(os.environ.get("DMPRD_MAX_EPOCH", "120"))
DONE_EPOCH = MAX_EPOCH - 1
POLL_SECONDS = int(os.environ.get("DMPRD_POLL_SECONDS", "600"))
LAUNCH_SETTLE_SECONDS = int(os.environ.get(
    "DMPRD_LAUNCH_SETTLE_SECONDS", "30"))
MIN_FREE_MIB = int(os.environ.get("DMPRD_MIN_FREE_MIB", "14000"))
MAX_UTIL = int(os.environ.get("DMPRD_MAX_UTIL", "15"))
OUT_DIR = Path(os.environ.get(
    "DMPRD_OUT_DIR", str(REPO / "results" / "dmprd_quick120")))
EVENTS = REPO / ".dmprd_quick120_queue.events"
ACTIVE_DIR = REPO / ".dmprd_quick120_active"
FAILED_DIR = REPO / ".dmprd_quick120_failed"
...
VARIANTS = {
    "a1_single": {
        "slots": 1,
        "distribution": False,
        "purpose": "single class prototype with bounded residual",
    },
    "a2_multi": {
        "slots": 4,
        "distribution": False,
        "purpose": "four class prototypes without distribution statistics",
    },
    "a3_full": {
        "slots": 4,
        "distribution": True,
        "purpose": "full distribution-aware multi-prototype residual",
    },
}

# The first four jobs occupy the four GPUs currently idle on the host. The
# remaining full-model datasets start automatically as GPUs become available.
TASKS = [
...
def run_stem(dataset, variant, seed):
    return f"AML-{dataset}-DMPRDQuick120-{variant}-Seed{seed}"
...
def launch(dataset, variant_name, seed, gpu):
    ...
    cmd = [
        ...
        "--repeat", "1", "--gpu", "0",
        "out_dir", str(OUT_DIR),
        "name_tag", f"DMPRDQuick120-{variant_name}-Seed{seed}",
        "seed", str(seed),
...
def main():
    log(
        f"DMPRD quick queue start max_epoch={MAX_EPOCH} "
        f"tasks={len(TASKS)}")
    while True:
        ...
        if not pending and not markers:
            ...
            log(f"DMPRD quick queue finished failures={failures}")
