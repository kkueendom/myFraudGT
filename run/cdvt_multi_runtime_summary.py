...
def validate(payload, path, dataset, variant):
    expected = {
...
def build_summary(runtime_root, datasets):
    ...
    for dataset in datasets:
        for variant in VARIANTS:
            ...
            validate(payload, path, dataset, variant)
    ...
    return {
        "phase": "CDVT_multi_runtime_quick_summary",
        "benchmark_mode": "normal_only_end_to_end_inference",
        "sampling_protocol": "dynamic_random",
...
def main():
    args = parse_args()
    summary = build_summary(args.runtime_root, args.datasets)
    if args.write:
