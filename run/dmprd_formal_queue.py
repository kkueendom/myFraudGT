#!/usr/bin/env python3
"""GPU queue for the 500-epoch DMPRD-A2 mainline screen.
...
MAX_EPOCH = int(os.environ.get("DMPRD_FORMAL_MAX_EPOCH", "500"))
DONE_EPOCH = MAX_EPOCH - 1
POLL_SECONDS = int(os.environ.get("DMPRD_FORMAL_POLL_SECONDS", "600"))
LAUNCH_SETTLE_SECONDS = int(os.environ.get(
    "DMPRD_FORMAL_LAUNCH_SETTLE_SECONDS", "30"))
MIN_FREE_MIB = int(os.environ.get(
    "DMPRD_FORMAL_MIN_FREE_MIB", "14000"))
MAX_UTIL = int(os.environ.get("DMPRD_FORMAL_MAX_UTIL", "15"))
OUT_DIR = Path(os.environ.get(
    "DMPRD_FORMAL_OUT_DIR", str(REPO / "results" / "dmprd_formal500")))
EVENTS = REPO / ".dmprd_formal500_queue.events"
ACTIVE_DIR = REPO / ".dmprd_formal500_active"
FAILED_DIR = REPO / ".dmprd_formal500_failed"
...
# A2 beat A1 and A3 on the representative Small-LI ablation. Run the selected
# route on all six datasets, prioritizing the slow large datasets at launch.
TASKS = [
...
def run_stem(dataset, variant, seed):
    return f"AML-{dataset}-DMPRDFormal500-{variant}-Seed{seed}"
...
def launch(dataset, variant_name, seed, gpu):
    ...
    cmd = [
        ...
        "--repeat", "1", "--gpu", "0",
        "out_dir", str(OUT_DIR),
        "name_tag", f"DMPRDFormal500-{variant_name}-Seed{seed}",
        "seed", str(seed),
...
def main():
    log(
        f"DMPRD formal queue start max_epoch={MAX_EPOCH} "
        f"tasks={len(TASKS)}")
    while True:
        ...
        if not pending and not markers:
            ...
            log(f"DMPRD formal queue finished failures={failures}")
