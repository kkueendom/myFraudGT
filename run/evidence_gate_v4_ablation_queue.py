#!/usr/bin/env python3
"""GPU queue for evidence-gate v4 ablations.
...
EVENTS = REPO / ".evidence_gate_v4_ablation_queue.events"
ACTIVE_DIR = REPO / ".evidence_gate_v4_ablation_active"
...
OUT_DIR = Path(os.environ.get("EVIDENCE_GATE_V4_ABLATION_OUT_DIR", str(REPO / "results" / "evidence_gate_v4_ablation")))
...
VARIANTS = [
    ...
    },
    {
        ...
        "purpose": "same structural residual but always-open gate",
    },
    {
        "name": "no_aux",
...
STAGE_A_VARIANTS = ["proto_only", "no_gate", "no_aux", "no_budget"]
...
def run_stem(dataset, variant, seed):
    return f"AML-{dataset}-V4Ablation-{variant}-Seed{seed}"
...
def launch(dataset, variant_name, seed, gpu):
    ...
    log_path = REPO / f".evidence_gate_v4_ablation_{run_name}.log"
    cmd = [
        ...
        "--gpu", "0",
        "out_dir", str(OUT_DIR),
        "name_tag", f"V4Ablation-{variant_name}-Seed{seed}",
        "seed", str(seed),
        "optim.max_epoch", "500",
