#!/usr/bin/env python3
"""GPU queue for two additional DMPRD-A2 seeds per dataset.
...
MAX_EPOCH = int(os.environ.get("DMPRD_A2_ROBUST_MAX_EPOCH", "500"))
DONE_EPOCH = MAX_EPOCH - 1
POLL_SECONDS = int(os.environ.get("DMPRD_A2_ROBUST_POLL_SECONDS", "600"))
LAUNCH_SETTLE_SECONDS = int(os.environ.get(
    "DMPRD_A2_ROBUST_LAUNCH_SETTLE_SECONDS", "30"))
MIN_FREE_MIB = int(os.environ.get(
    "DMPRD_A2_ROBUST_MIN_FREE_MIB", "14000"))
MAX_UTIL = int(os.environ.get("DMPRD_A2_ROBUST_MAX_UTIL", "15"))
OUT_DIR = Path(os.environ.get(
    "DMPRD_A2_ROBUST_OUT_DIR",
    str(REPO / "results" / "dmprd_a2_robust500")))
EVENTS = REPO / ".dmprd_a2_robust500_queue.events"
ACTIVE_DIR = REPO / ".dmprd_a2_robust500_active"
FAILED_DIR = REPO / ".dmprd_a2_robust500_failed"
...
VARIANTS = {
    "a2_multi": {
        "slots": 4,
        "distribution": False,
        "purpose": "selected four-prototype bounded residual",
    },
}

# The best-seed formal run supplies one of seeds 42/43/44. These tasks fill in
# the other two seeds. The first wave covers every dataset before third seeds.
TASKS = [
    ("Small-LI", "a2_multi", 43),
    ("Medium-HI", "a2_multi", 43),
    ("Large-LI", "a2_multi", 42),
    ("Large-HI", "a2_multi", 42),
    ("Medium-LI", "a2_multi", 42),
    ("Small-HI", "a2_multi", 43),
    ("Small-LI", "a2_multi", 44),
    ("Medium-HI", "a2_multi", 44),
    ("Large-LI", "a2_multi", 43),
    ("Large-HI", "a2_multi", 44),
    ("Medium-LI", "a2_multi", 43),
    ("Small-HI", "a2_multi", 44),
]
...
def run_stem(dataset, variant, seed):
    return f"AML-{dataset}-DMPRDA2Robust500-{variant}-Seed{seed}"
...
def launch(dataset, variant_name, seed, gpu):
    ...
    stdout_path = REPO / f".dmprd_a2_robust500_{run_name}_gpu{gpu}.log"
    cmd = [
        PYTHON, "-m", "fraudGT.main",
        "--cfg", f"configs/evidence_gate_v4/AML-{dataset}.yaml",
        "--repeat", "1", "--gpu", "0",
        "out_dir", str(OUT_DIR),
        "name_tag", f"DMPRDA2Robust500-{variant_name}-Seed{seed}",
        "seed", str(seed),
...
def main():
    log(
        f"DMPRD A2 robustness queue start max_epoch={MAX_EPOCH} "
        f"tasks={len(TASKS)}")
    while True:
        ...
        if not pending and not markers:
            ...
            log(f"DMPRD A2 robustness queue finished failures={failures}")
