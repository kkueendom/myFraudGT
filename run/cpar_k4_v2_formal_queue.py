...
def configure_paths(git_sha):
    ...
    EVENTS = REPO / f".{namespace}_queue.events"
    ACTIVE_DIR = REPO / f".{namespace}_active"
    FAILED_DIR = REPO / f".{namespace}_failed"
...
def launch(dataset, seed, gpu, git_sha):
    ...
    stdout_path = (
        REPO / f".cpar_k4_{VERSION}_{git_sha}_{run_name}_gpu{gpu}.log")
    cmd = [
        ...
        "model.cpar_gap_threshold", "0.005",
