#!/usr/bin/env python3
"""Audit one DABR strength against matched A2 trajectories."""
...
def dabr_run(root, dataset, seed, epoch_limit, commit, strength_tag):
    commit_pattern = f"{commit}*" if commit else "*"
    stem = (
        f"AML-{dataset}-DABR{strength_tag}Formal500-full-Seed{seed}-"
        f"{commit_pattern}-gpu*")
    # The second pattern permits deliberate OUT_DIR/name-tag overrides while
    # still requiring DABR, strength, and matched dataset/seed identifiers.
    if commit:
        fallback = (
            f"AML-{dataset}-*DABR{strength_tag}*full*Seed{seed}*"
            f"{commit}*-gpu*")
    else:
        fallback = (
            f"AML-{dataset}-*DABR{strength_tag}*full*Seed{seed}*-gpu*")
    return find_run(root, [stem, fallback], seed, epoch_limit)


def current_short_sha(repo):
    result = subprocess.run(
        ["git", "rev-parse", "--short=8", "HEAD"],
        cwd=str(repo), text=True, capture_output=True, check=False)
    if result.returncode != 0 or not result.stdout.strip():
        raise RuntimeError("cannot determine DABR commit")
...
def main():
    ...
    parser.add_argument(
        ...
        help="short DABR commit embedded in result root and run names")
    parser.add_argument("--epoch-limit", type=int, default=499)
    parser.add_argument("--min-delta", type=float, default=0.005)
    args = parser.parse_args()

    dabr_root = (
        Path(args.dabr) if args.dabr else
        repo / "results" /
        f"dabr_{args.strength_tag}_{default_commit}_formal500")
    a2_root = Path(args.a2)
    print(
        "dataset\tseed\ta2_last\tdabr_last\ta2_val_f1\tdabr_val_f1"
        "\ta2_selected_test\tdabr_selected_test\tdelta_selected_test"
        "\ta2_raw_best\tdabr_raw_best\tdelta_raw\ta2_val_epoch"
        "\tdabr_val_epoch\ta2_raw_epoch\tdabr_raw_epoch")

    paired = []
    for dataset, seed in TASKS:
        a2 = a2_run(a2_root, dataset, seed, args.epoch_limit)
        dabr = dabr_run(
            dabr_root, dataset, seed, args.epoch_limit, args.commit,
            args.strength_tag)
        a2_selected = metric(a2, "test_at_val")
        dabr_selected = metric(dabr, "test_at_val")
        a2_raw = metric(a2, "raw_best")
        dabr_raw = metric(dabr, "raw_best")
        selected_delta = delta(dabr_selected, a2_selected)
        raw_delta = delta(dabr_raw, a2_raw)
        complete = (
            a2 is not None and dabr is not None and
            a2["last_epoch"] >= args.epoch_limit and
            dabr["last_epoch"] >= args.epoch_limit and
        ...
        print("\t".join([
            ...
            str(dabr["last_epoch"]) if dabr else "-",
            fmt(metric(a2, "val_f1")),
            fmt(metric(dabr, "val_f1")),
            fmt(a2_selected),
            fmt(dabr_selected),
            fmt(selected_delta),
            fmt(a2_raw),
            ...
            str(dabr["val_epoch"]) if dabr else "-",
            str(a2["raw_epoch"]) if a2 else "-",
            str(dabr["raw_epoch"]) if dabr else "-",
        ]))

    print(
        f"dabr_pair_go_no_go strength={args.strength_tag} "
    ...
    if not formal_complete:
        print("dabr_formal_pair_screen=INCOMPLETE")
    elif wins == 2:
        print("dabr_formal_pair_screen=PASS")
    else:
        print("dabr_formal_pair_screen=FAIL")
