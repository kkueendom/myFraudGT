#!/usr/bin/env python3
"""Audit DMPRD-A3 against A2, proto_only, and historical baseline."""
...
def formal_run(root, dataset, variant, seed, epoch_limit):
    stem = f"AML-{dataset}-DMPRDA3Formal500-{variant}-Seed{seed}-gpu*"
...
def a2_reference(root, dataset, seed, epoch_limit):
...
def main():
    ...
    parser.add_argument(
        "--out", default=str(
            Path(__file__).resolve().parents[1] / "results" / "dmprd_a3_formal500"))
    parser.add_argument(
        "--proto-reference",
        default=(
    ...
    a2_root = Path(args.a2_reference)
    print(
        ...
        "\ta2_val\ta2_raw\tdelta_val_vs_a2\tdelta_raw_vs_a2")
    proto_deltas = []
    baseline_deltas = []
    a2_deltas = []
    for dataset, variant, seed in TASKS:
        ...
        a2 = a2_reference(
            a2_root, dataset, seed, args.epoch_limit)
        ...
        delta_a2_val = (
            current_val - a2_val
            if current_val is not None and a2_val is not None else None)
        delta_a2_raw = (
            current_raw - a2_raw
            if current_raw is not None and a2_raw is not None else None)
        ...
        if delta_a2_val is not None and delta_a2_raw is not None:
            a2_deltas.append((dataset, delta_a2_val, delta_a2_raw))
        print("\t".join([
            dataset,
            variant,
            str(seed),
    ...
    if proto_deltas:
        ...
        print(
            f"a3_matched={len(proto_deltas)}/6 "
    ...
    if a2_deltas:
        mean_val = sum(item[1] for item in a2_deltas) / len(a2_deltas)
        mean_raw = sum(item[2] for item in a2_deltas) / len(a2_deltas)
        val_wins = sum(1 for item in a2_deltas if item[1] > 0.0)
        raw_wins = sum(1 for item in a2_deltas if item[2] > 0.0)
        print(
            f"a2_reference_matched={len(a2_deltas)}/6 "
            f"mean_delta_val_vs_a2={mean_val:+.5f} "
            f"mean_delta_raw_vs_a2={mean_raw:+.5f} "
            f"val_wins={val_wins}/6 raw_wins={raw_wins}/6")
        if len(a2_deltas) == 6:
            contributes = mean_val > 0.0 and mean_raw > 0.0 and raw_wins >= 4
            print(
                "distribution_stats_contribute="
                f"{'PASS' if contributes else 'FAIL'}")
