#!/usr/bin/env python3
"""Audit COSTAR Small-LI/Large-LI runs against matched A2 trajectories."""
...
def costar_run(root, dataset, seed, epoch_limit, commit):
    commit_pattern = f"{commit}*" if commit else "*"
    stem = (
        f"AML-{dataset}-COSTARFormal500-full-Seed{seed}-"
        f"{commit_pattern}-gpu*")
    # The second pattern permits deliberate OUT_DIR/name-tag overrides while
    # still requiring the COSTAR method and matched dataset/seed identifiers.
    if commit:
        fallback = (
            f"AML-{dataset}-*COSTAR*full*Seed{seed}*{commit}*-gpu*")
    else:
        fallback = f"AML-{dataset}-*COSTAR*full*Seed{seed}*-gpu*"
...
def main():
    ...
    parser.add_argument(
        "--costar", default=str(repo / "results" / "costar_formal500"))
    parser.add_argument(
        "--a2", default=str(default_a2))
    parser.add_argument(
        "--commit", default="",
        help="optional short commit prefix embedded in COSTAR run names")
    parser.add_argument("--epoch-limit", type=int, default=499)
    parser.add_argument("--min-delta", type=float, default=0.005)
    args = parser.parse_args()

    costar_root = Path(args.costar)
    a2_root = Path(args.a2)
    print(
        "dataset\tseed\ta2_last\tcostar_last\ta2_val_f1\tcostar_val_f1"
        "\ta2_selected_test\tcostar_selected_test\tdelta_selected_test"
        "\ta2_raw_best\tcostar_raw_best\tdelta_raw\ta2_val_epoch"
        "\tcostar_val_epoch\ta2_raw_epoch\tcostar_raw_epoch")

    paired = []
    for dataset, seed in TASKS:
        a2 = a2_run(a2_root, dataset, seed, args.epoch_limit)
        costar = costar_run(
            costar_root, dataset, seed, args.epoch_limit, args.commit)
        a2_selected = metric(a2, "test_at_val")
        costar_selected = metric(costar, "test_at_val")
        a2_raw = metric(a2, "raw_best")
        costar_raw = metric(costar, "raw_best")
        selected_delta = delta(costar_selected, a2_selected)
        raw_delta = delta(costar_raw, a2_raw)
        complete = (
            a2 is not None and costar is not None and
            a2["last_epoch"] >= args.epoch_limit and
            costar["last_epoch"] >= args.epoch_limit and
        ...
        print("\t".join([
            ...
            str(costar["last_epoch"]) if costar else "-",
            fmt(metric(a2, "val_f1")),
            fmt(metric(costar, "val_f1")),
            fmt(a2_selected),
            fmt(costar_selected),
            fmt(selected_delta),
            fmt(a2_raw),
            ...
            str(costar["val_epoch"]) if costar else "-",
            str(a2["raw_epoch"]) if a2 else "-",
            str(costar["raw_epoch"]) if costar else "-",
        ]))

    print("costar_pair_go_no_go")
    if paired:
