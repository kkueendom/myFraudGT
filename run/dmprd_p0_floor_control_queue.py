#!/usr/bin/env python3
"""GPU queue for the P0 fixed-floor confound and seed-43 controls."""
...
MAX_EPOCH = int(os.environ.get("DMPRD_P0_MAX_EPOCH", "120"))
DONE_EPOCH = MAX_EPOCH - 1
POLL_SECONDS = int(os.environ.get("DMPRD_P0_POLL_SECONDS", "600"))
...
OUT_DIR = Path(os.environ.get(
    "DMPRD_P0_OUT_DIR", str(
        REPO / "results" / "dmprd_p0_floor_control120")))
EVENTS = REPO / ".dmprd_p0_floor_control120_queue.events"
ACTIVE_DIR = REPO / ".dmprd_p0_floor_control120_active"
FAILED_DIR = REPO / ".dmprd_p0_floor_control120_failed"
...
# Three fixed-floor controls test the main confound. The remaining GPUs provide
# one additional matched A2/P0 seed on Small-LI without delaying the controls.
TASKS = [
    ("Large-LI", "floor025_control", 44),
    ("Medium-HI", "floor025_control", 42),
    ("Small-LI", "floor025_control", 42),
    ("Small-LI", "a2_seed43", 43),
    ("Small-LI", "p0_seed43", 43),
]
...
def run_stem(dataset, variant, seed):
    return f"AML-{dataset}-DMPRDP0FloorControl120-{variant}-Seed{seed}"
...
def launch(dataset, variant_name, seed, gpu):
    ...
    stdout_path = REPO / f".dmprd_p0_floor_control120_{run_name}_gpu{gpu}.log"
    cmd = [
        PYTHON, "-m", "fraudGT.main",
        "--cfg", f"configs/evidence_gate_v4/AML-{dataset}.yaml",
        "--repeat", "1", "--gpu", "0",
        "out_dir", str(OUT_DIR),
        "name_tag", f"DMPRDP0FloorControl120-{variant_name}-Seed{seed}",
        "seed", str(seed),
        ...
        "model.dmprd_margin_tau", str(variant["margin_tau"]),
...
def main():
    log(
        f"DMPRD P0 floor-control queue start max_epoch={MAX_EPOCH} "
    ...
    while True:
        ...
        if not pending and not markers:
            ...
            log(f"DMPRD P0 floor-control queue finished failures={failures}")
