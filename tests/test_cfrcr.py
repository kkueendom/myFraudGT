...
def test_margin_correction_changes_only_binary_margin():
    logits = torch.tensor([[1.0, 3.0], [-2.0, 4.0]])
    correction = torch.tensor([0.4, -0.2])
    output = _add_margin_correction(logits, correction)

    assert torch.allclose(output.mean(dim=-1), logits.mean(dim=-1))
    assert torch.allclose(
        _binary_margin(output), _binary_margin(logits) + correction)


def test_risk_controlled_committee_falls_back_on_disagreement():
    ...
    assert torch.allclose(mean, torch.tensor([0.30, 0.00, 0.02]))
    assert torch.allclose(committed, torch.tensor([0.29, 0.00, 0.01]))
    assert disagreement[1] > disagreement[0]


def test_pair_regret_has_a_finite_gain_stop():
    ...
    assert improved_valid and worse_valid
    assert improved_regret.item() == 0.0
    assert worse_regret > 0.0
...
def test_each_expert_receives_no_label_gradient_from_held_out_environment():
    module = _DummyCFRCR()
    ...
    for expert_idx in range(3):
        ...
        included = ~held_out
        assert torch.count_nonzero(gradients[held_out, expert_idx]) == 0
        assert torch.count_nonzero(gradients[included, expert_idx]) > 0
